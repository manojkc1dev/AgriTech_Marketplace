...
export default function KycVerificationModal({
  ...
  const [citizenshipDocUrl, setCitizenshipDocUrl] = useState(
    user.citizenshipDocUrl || ""
  );
  const [nationalIdNumber, setNationalIdNumber] = useState(
    user.nationalIdNumber || ""
  );
  const [nationalIdDocUrl, setNationalIdDocUrl] = useState(
    user.nationalIdDocUrl || ""
  );

  const [citizenshipFileName, setCitizenshipFileName] =
    useState<string>("citizenship_scan.jpg");
  const [nationalIdFileName, setNationalIdFileName] =
    useState<string>("national_id_card.jpg");

  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [successMsg, setSuccessMsg] = useState<string | null>(null);

  // Sync state during render when prop changes (React recommended pattern replacing useEffect)
  if (user !== prevUser) {
    setPrevUser(user);
  ...
  const handleSubmitKyc = async (e: FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setSuccessMsg(
          "Your Citizenship & National Identity Card documents have been submitted! Verification is now pending Admin / Super Admin review."
        );
  ...
  const status =
    user.verificationStatus || (user.verified ? "verified" : "unverified");
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-100/80 mt-0.5">
                नागरिकता तथा राष्ट्रिय परिचयपत्र प्रमाणीकरण (First Buy & Sell
                Requirement)
              </p>
            </div>
          </div>
            ...
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-start space-x-3 text-emerald-800">
              ...
              <div>
                <h4 className="font-bold text-sm">
                  Account Verified by Super Admin!
                </h4>
                <p className="text-xs text-emerald-700 mt-1 leading-relaxed">
                  ...
                  ) and National Identity Card (NIN:{" "}
                  <strong className="font-mono">
                    {user.nationalIdNumber}
                  </strong>
                  ) have been officially approved. You have full authorized access
                  to buy and sell produce across all districts.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start space-x-3 text-amber-900">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-amber-800 leading-relaxed">
                  Your Citizenship Card & National Identity Card documents were
                ...
                <p className="text-[11px] text-amber-700 italic">
                  Note: A Super Admin or Admin will review your credentials
                  shortly. You will receive immediate access to buy & sell upon
                  sign-off.
                </p>
              </div>
            </div>
              ...
              <div>
                ...
                <p className="text-xs text-rose-700 mt-1 leading-relaxed">
                  {user.verificationNotes ||
                    "The uploaded Citizenship or National Identity Card image was incomplete or unreadable. Please re-upload clearer photos."}
                </p>
              </div>
            </div>
          )}
          ...
          {status !== "verified" && status !== "pending" && (
            <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl space-y-2">
              ...
              <p className="text-xs text-slate-600 leading-relaxed">
                To prevent fraud, guarantee harvest quality, and protect direct
                financial payouts for Nepalese smallholders and commercial
                buyers, all users are required to upload both their{" "}
                <strong>Citizenship Card (नागरिकता)</strong> and{" "}
                <strong>National Identity Card (राष्ट्रिय परिचयपत्र)</strong> on
                their first purchase or sale. Trading activates immediately
                following approval from a Super Admin or District Admin.
              </p>
            </div>
          )}
            ...
            <form onSubmit={handleSubmitKyc} className="space-y-5">
              ...
              <div className="flex justify-between items-center bg-emerald-50 border border-emerald-200 p-3 rounded-xl">
                <div className="flex items-center space-x-2 text-emerald-800 text-xs font-semibold">
                  ...
                  <span>
                    Testing mode active: Auto-fill official sample documents?
                  </span>
                </div>
                <button
                  type="button"
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-3 shadow-sm">
                <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                  <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center space-x-1.5">
                    ...
                    <span>
                      2. National Identity Card (राष्ट्रिय परिचयपत्र - NIN)
                    </span>
                  </h3>
                  <span className="text-[10px] text-rose-600 font-bold uppercase">
                    * Mandatory
