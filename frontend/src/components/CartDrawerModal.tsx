...
export default function CartDrawerModal({ user, token, onNavigateToOrders }: CartDrawerModalProps) {
  const { t } = useLanguage();
  const {
    cart,
    removeFromCart,
  ...
  const [selectedPayment, setSelectedPayment] = useState<"esewa" | "khalti" | "connectips" | "cod">("esewa");
  const [qrModalFarmer, setQrModalFarmer] = useState<{ farmerId: string; farmerName: string; amount: number } | null>(null);
  ...
  const handleCheckoutFarmer = async (farmerId: string, farmerName: string, subtotal: number) => {
    // Generate order or show QR
  ...
  const handleCompletePaymentAndOrder = async () => {
    ...
    try {
      ...
      setOrderSuccessMsg(`Order successfully placed with ${qrModalFarmer.farmerName}! Payment via ${selectedPayment.toUpperCase()} verified.`);
      setQrModalFarmer(null);
      // Remove farmer items from cart
      const farmerItems = cart.filter(i => i.farmerId === qrModalFarmer.farmerId);
            ...
            <div className="flex-1">
              ...
              {onNavigateToOrders && (
                <button
                  ...
                  className="block text-emerald-700 dark:text-emerald-300 underline font-bold mt-1"
                >
          ...
          {cart.length === 0 ? (
            <div className="space-y-6">
              <div className="text-center py-10 space-y-4">
                ...
                <div className="space-y-1">
                  <h3 className="font-bold text-slate-800 dark:text-slate-200 text-base">{t("Your Cart is Empty")}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 max-w-xs mx-auto leading-relaxed">
                    {t("Browse live market listings from Dhading, Makwanpur, and Kathmandu farming cooperatives to add crops to your cart.")}
                  </p>
                </div>
              </div>

              {/* Coupons & Promo Codes Section (Always available in Cart) */}
              <div className="bg-amber-50/70 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/60 rounded-2xl p-4 space-y-3">
                ...
                {appliedCoupon ? (
                  <div className="bg-emerald-100 dark:bg-emerald-900/60 border border-emerald-300 dark:border-emerald-700 text-emerald-900 dark:text-emerald-100 p-2.5 rounded-xl text-xs flex items-center justify-between">
                    <div className="space-y-0.5">
                      ...
                      <span className="text-[11px] block text-emerald-800 dark:text-emerald-300">
                        {appliedCoupon.description} &bull; Savings: NRs. {discountValue.toLocaleString()}
                      </span>
                    </div>
                    <button
                  ...
                  <form onSubmit={handleApplyCoupon} className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-1.5 text-xs bg-white dark:bg-slate-800 border border-amber-300 dark:border-amber-700 rounded-xl outline-none focus:ring-1 focus:ring-amber-500 uppercase font-mono font-bold"
                    />
                    <button
                      type="submit"
                ...
                {couponMessage && (
                  <p className={`text-[11px] font-semibold ${couponMessage.success ? "text-emerald-700 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`}>
                    {couponMessage.text}
                  </p>
                )}
                    ...
                    {AVAILABLE_COUPONS.map((c) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400 hover:underline flex items-center space-x-1 ml-2"
                        >
                          {copiedCoupon === c.code ? (
                            <>
            ...
            <>
              ...
              <div className="space-y-4">
                {(Object.entries(farmerGroups) as [string, FarmerGroup][]).map(([farmerId, group]) => {
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <div>
                            ...
                            <span className="text-[10px] text-slate-500 dark:text-slate-400">
                              {group.district || "Nepal"} &bull; {group.items.length} item(s)
                            </span>
                          </div>
                        </div>
                          ...
                          <div key={item.id} className="py-2.5 flex items-center justify-between text-xs gap-2">
                            ...
                            <div className="flex items-center space-x-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-0.5 shadow-2xs">
                              <button
                                ...
                                className="p-1 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded transition"
                      ...
                      <div className="pt-1 flex justify-end">
                        <button
                          onClick={() => handleCheckoutFarmer(farmerId, group.farmerName, farmerTotal)}
                        ...
                        >
                          ...
                          <span>Pay {group.farmerName} (NRs. {farmerTotal.toLocaleString()})</span>
                        </button>
                      </div>
              ...
              <div className="bg-slate-50 dark:bg-slate-850 border border-slate-200 dark:border-slate-750 rounded-2xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  ...
                  >
                    <span className="text-xs block font-extrabold text-emerald-600">eSewa</span>
                    <span className="text-[9px] text-slate-400 block">Digital Wallet</span>
                  </button>
                  ...
                  >
                    <span className="text-xs block font-extrabold text-purple-600">Khalti</span>
                    <span className="text-[9px] text-slate-400 block">Instant Pay</span>
                  </button>
                  ...
                  >
                    <span className="text-xs block font-extrabold text-blue-600">ConnectIPS</span>
                    <span className="text-[9px] text-slate-400 block">Direct Bank</span>
                  </button>
                  ...
                  >
                    <span className="text-xs block font-extrabold text-amber-600">Cash/COD</span>
                    <span className="text-[9px] text-slate-400 block">On Delivery</span>
                  </button>
                </div>
              </div>
            </>
          ...
          <div className="p-5 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-850 shrink-0 space-y-3">
            <div className="space-y-1.5 text-xs text-slate-600 dark:text-slate-300">
              <div className="flex justify-between">
                ...
                <span className="font-mono font-semibold">NRs. {totalCartValue.toLocaleString()}</span>
              </div>

              {discountValue > 0 && (
                <div className="flex justify-between text-emerald-700 dark:text-emerald-400 font-semibold">
                  <span>Discount ({appliedCoupon?.code}):</span>
                  <span className="font-mono">- NRs. {discountValue.toLocaleString()}</span>
                </div>
              )}
            ...
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={() => {
                  const firstFarmer = (Object.entries(farmerGroups) as [string, FarmerGroup][])[0];
                  if (firstFarmer) {
                    const farmerTotal = firstFarmer[1].items.reduce((s, i) => s + i.quantity * i.pricePerUnit, 0);
                    handleCheckoutFarmer(firstFarmer[0], firstFarmer[1].farmerName, farmerTotal);
                  }
                }}
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-sm w-full p-6 shadow-2xl space-y-5 text-center">
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-3">
              ...
              <button onClick={() => setQrModalFarmer(null)} className="text-slate-400 hover:text-slate-600">
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="space-y-1">
              <p className="text-xs text-slate-500">Pay directly to registered farmer:</p>
              <h3 className="font-extrabold text-base text-slate-900 dark:text-white">{qrModalFarmer.farmerName}</h3>
            ...
            <p className="text-[11px] text-slate-500">
              Scan with <strong className="text-emerald-700">{selectedPayment.toUpperCase()}</strong> or bank mobile app to settle order.
            </p>
