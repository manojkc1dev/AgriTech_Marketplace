...
import { User, ProduceListing, Order, Negotiation, Cooperative, MarketPrice, SoilLog, HarvestRecord, PriceAlert, PriceNotification, CooperativeMessage, CooperativeAnnouncement, ScanHistoryItem } from "../types";
import { Plus, Check, X, Send, History, Building2, UserCheck, AlertTriangle, ChevronDown, ChevronUp, DollarSign, Leaf, ShoppingBag, Sprout, TrendingUp, Info, BarChart3, TrendingDown, Sun, Cloud, CloudRain, CloudLightning, Thermometer, Droplets, Wind, Calendar, AlertCircle, ThumbsUp, ThumbsDown, CloudSun, ClipboardList, Activity, FlaskConical, Bell, Trash2, BellRing, Settings, ShieldCheck, CheckSquare, RefreshCw, Sparkles, Megaphone, Calculator, Camera, QrCode, Tag, Search, Filter, MapPin, CheckCircle2 } from "lucide-react";
...
const CROP_BASE_STATS: Record<string, { baseYield: number; defaultPrice: number }> = {
...
const generateForecast = (district: string): WeatherDay[] => {
  const conditions: { type: string; humidity: number; rainProb: number; icon: "sun" | "sun-cloud" | "rain" | "storm" | "cloud" }[] = [
  ...
  return Array.from({ length: 7 }).map((_, idx) => {
    ...
    const cycleIndex = (idx + (district ? district.charCodeAt(0) : 7)) % conditions.length;
...
const getHarvestSuitability = (crop: string, weather: WeatherDay): SuitabilityResult => {
...
export default function FarmerDashboard({ user, token, onUserUpdate }: FarmerProps) {
  ...
  const [activeTab, setActiveTab] = useState<"listings" | "orders" | "coops" | "predictions" | "climate" | "soil" | "forecasting" | "alerts" | "advisor" | "calculator" | "scans">("listings");
  ...
  const [scanFilter, setScanFilter] = useState<"all" | "member" | "batch" | "text">("all");
  ...
  const handleClearScanHistory = async () => {
    if (!window.confirm("Are you sure you want to clear your entire QR scan history? This action cannot be undone.")) {
      return;
    }
    try {
  ...
  const [weatherAlertSeverity, setWeatherAlertSeverity] = useState<"extreme" | "severe" | "moderate" | "normal">("extreme");
  const [weatherAlertDistrict, setWeatherAlertDistrict] = useState<string>(user.district || "Dhading");
  ...
  const [showWeatherBanner, setShowWeatherBanner] = useState(() => {
    try {
      ...
      if (saved) {
        ...
        const isWithin24Hours = Date.now() - (parsed.timestamp || 0) < TWENTY_FOUR_HOURS;
        if (isWithin24Hours && parsed.severity === "extreme" && parsed.district === (user.district || "Dhading")) {
          return false;
        }
      }
    } catch {
  ...
  const handleWeatherSeverityChange = (newSeverity: "extreme" | "severe" | "moderate" | "normal") => {
  ...
  const [printableListing, setPrintableListing] = useState<ProduceListing | null>(null);
  ...
  const handleQrPreFillForm = (crop: string, qty: string, price: string) => {
    ...
    setNewQty(qty);
    setNewPrice(price);
    setFormSuccess(t("Form pre-filled from scanned QR code! Click 'Publish Listing' to confirm."));
  };
  ...
  const handleMarkNotificationRead = async (id: string, isRead: boolean = true) => {
    try {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.triggeredCount > 0) {
          setSimulationResult(`Success! ${data.triggeredCount} alert(s) simulated & triggered.`);
        } else {
          setSimulationResult("Check complete: No active alert criteria were met by this price.");
        }
        fetchAlertsAndNotifications();
      } else {
        const err = await res.json();
  ...
  const [calcLandUnit, setCalcLandUnit] = useState<"ropani" | "bigha" | "kattha" | "hectare" | "acre">("ropani");
  ...
  const [calcExpenseFertilizer, setCalcExpenseFertilizer] = useState<number>(2400);
  ...
  const [calcExpenseTransport, setCalcExpenseTransport] = useState<number>(1200);

  // Crop configuration presets (based on 1 Ropani)
  const calcCropPresets: Record<string, {
    name: string;
    nepaliName: string;
    yieldPerRopani: number;
    pricePerKg: number;
    expensesPerRopani: { seeds: number; fertilizer: number; labor: number; tractor: number; transport: number };
  }> = {
    Potato: {
      name: "Potato",
      ...
      yieldPerRopani: 600,
      pricePerKg: 45,
      expensesPerRopani: { seeds: 1200, fertilizer: 800, labor: 1500, tractor: 600, transport: 400 }
    },
    Tomato: {
      name: "Tomato",
      ...
      yieldPerRopani: 900,
      pricePerKg: 75,
      expensesPerRopani: { seeds: 1500, fertilizer: 1000, labor: 2000, tractor: 600, transport: 500 }
    },
    Rice: {
      name: "Rice/Paddy",
      ...
      yieldPerRopani: 250,
      pricePerKg: 35,
      expensesPerRopani: { seeds: 600, fertilizer: 700, labor: 1200, tractor: 800, transport: 300 }
    },
    Cauliflower: {
      name: "Cauliflower",
      ...
      yieldPerRopani: 750,
      pricePerKg: 65,
      expensesPerRopani: { seeds: 1000, fertilizer: 900, labor: 1800, tractor: 600, transport: 450 }
    },
    Cabbage: {
      name: "Cabbage",
      ...
      yieldPerRopani: 850,
      pricePerKg: 30,
      expensesPerRopani: { seeds: 800, fertilizer: 800, labor: 1600, tractor: 600, transport: 450 }
    },
    Ginger: {
      name: "Ginger",
      ...
      yieldPerRopani: 400,
      pricePerKg: 120,
      expensesPerRopani: { seeds: 3000, fertilizer: 1200, labor: 2200, tractor: 700, transport: 600 }
    },
    Onion: {
      name: "Onion",
      ...
      yieldPerRopani: 650,
      pricePerKg: 85,
      expensesPerRopani: { seeds: 1400, fertilizer: 900, labor: 1700, tractor: 600, transport: 400 }
    }
  };
  ...
  const handleApplyCropPreset = (cropKey: string, customLandUnit?: "ropani" | "bigha" | "kattha" | "hectare" | "acre") => {
    ...
    const liveMatch = marketPrices.filter(p => p.crop.toLowerCase().includes(preset.name.toLowerCase()));
    ...
    setCalcExpenseSeeds(Math.round(preset.expensesPerRopani.seeds * multiplier * calcLandSize));
    setCalcExpenseFertilizer(Math.round(preset.expensesPerRopani.fertilizer * multiplier * calcLandSize));
    setCalcExpenseLabor(Math.round(preset.expensesPerRopani.labor * multiplier * calcLandSize));
    setCalcExpenseTractor(Math.round(preset.expensesPerRopani.tractor * multiplier * calcLandSize));
    setCalcExpenseTransport(Math.round(preset.expensesPerRopani.transport * multiplier * calcLandSize));
  };
  ...
  const fetchCoopMessages = async () => {
    try {
      const res = await fetch("/api/cooperatives/messages", {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleOpenContactModal = (crop: string) => {
    setContactCrop(crop);
    setContactMessage(`Hello, we are looking for support to market/transport our listed crop: ${crop}. We'd like to coordinate with the cooperative.`);
    setCoopMsgSuccess("");
    setCoopMsgError("");
  ...
  const handleSendCoopMessage = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/cooperatives/contact", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
      ...
      if (res.ok) {
        setCoopMsgSuccess("Your message was sent to the cooperative successfully!");
        setContactMessage("");
        fetchCoopMessages();
        setTimeout(() => {
  ...
  const [smsDemandAlerts, setSmsDemandAlerts] = useState(!!user.smsDemandAlerts);
  const [inAppDemandAlerts, setInAppDemandAlerts] = useState(!!user.inAppDemandAlerts);
  const [smsWeatherAlerts, setSmsWeatherAlerts] = useState(!!user.smsWeatherAlerts);
  const [inAppWeatherAlerts, setInAppWeatherAlerts] = useState(!!user.inAppWeatherAlerts);
  ...
  const handleSaveNotifications = async (
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json();
        setNotificationError(err.error || "Failed to update notification settings.");
      }
    } catch (e) {
  ...
  const toggleSmsDemand = () => {
    ...
    handleSaveNotifications(nextVal, inAppDemandAlerts, smsWeatherAlerts, inAppWeatherAlerts);
  };
  const toggleInAppDemand = () => {
    const nextVal = !inAppDemandAlerts;
    setInAppDemandAlerts(nextVal);
    handleSaveNotifications(smsDemandAlerts, nextVal, smsWeatherAlerts, inAppWeatherAlerts);
  };
  const toggleSmsWeather = () => {
    const nextVal = !smsWeatherAlerts;
    setSmsWeatherAlerts(nextVal);
    handleSaveNotifications(smsDemandAlerts, inAppDemandAlerts, nextVal, inAppWeatherAlerts);
  };
  const toggleInAppWeather = () => {
    const nextVal = !inAppWeatherAlerts;
    setInAppWeatherAlerts(nextVal);
    handleSaveNotifications(smsDemandAlerts, inAppDemandAlerts, smsWeatherAlerts, nextVal);
  };
  
  // Soil & Nutrient Management States
  const [soilLogs, setSoilLogs] = useState<SoilLog[]>([]);
  const [soilCropBatch, setSoilCropBatch] = useState("Tomato (Golbheda) - Block A");
  const [soilLogType, setSoilLogType] = useState<"fertilizer" | "soil_test">("fertilizer");
  ...
  const [harvestWeather, setHarvestWeather] = useState<'optimal' | 'dry' | 'excessive'>("optimal");
  const [harvestSoil, setHarvestSoil] = useState<'poor' | 'organic' | 'balanced'>("balanced");
  ...
  const [simWeather, setSimWeather] = useState<'optimal' | 'dry' | 'excessive'>("optimal");
  const [simSoil, setSimSoil] = useState<'poor' | 'organic' | 'balanced'>("balanced");
  ...
  const handleGenerateAIForecast = async () => {
    ...
    setAiError("");
    try {
      const res = await fetch("/api/ai/forecast-harvest", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
      ...
      console.error(err);
      setAiError(err.message || "Failed to establish secure AI link with agronomy forecaster.");
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const handleGenerateAIAdvisor = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const errData = await res.json();
        throw new Error(errData.error || "Failed to generate AI advisor suggestions");
      }
      ...
      console.error(err);
      setAdvisorError(err.message || "Failed to establish secure link with AI Climatologist.");
    } finally {
  ...
  const [historyRegion, setHistoryRegion] = useState<"all" | "Kathmandu" | "Terai" | "Hill">("all");
  ...
  const fetchFarmerData = async () => {
    try {
      ...
      const listRes = await fetch(`/api/listings?farmerId=${user.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (listRes.ok) {
      ...
      if (coopRes.ok) {
        ...
        const relevantCoops = allCoops.filter((c: Cooperative) => c.district === user.district);
      ...
      const soilRes = await fetch("/api/soil-logs", {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (soilRes.ok) {
      ...
      const harvestRes = await fetch("/api/harvest-records", {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (harvestRes.ok) {
  ...
  const handleCreateListing = async (e: React.FormEvent) => {
    ...
    if (!newQty || Number(newQty) <= 0 || !newPrice || Number(newPrice) <= 0) {
      setFormError("Please enter valid positive numbers for quantity and target price.");
      return;
    }
  ...
  const handleCreateSoilLog = async (e: React.FormEvent) => {
    ...
    if (!soilDetails.trim()) {
      setSoilError("Please enter details or comments about this application/test.");
      return;
    }

    setIsSubmittingSoil(true);
    try {
      const res = await fetch("/api/soil-logs", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
  ...
  const handleCreateHarvestRecord = async (e: React.FormEvent) => {
    ...
    if (!harvestFertilizer.trim()) {
      setHarvestError("Please specify any fertilizer or nutrient application used.");
      return;
    }
  ...
  const predictionStats = React.useMemo(() => {
    const cropStats = CROP_BASE_STATS[predCrop] || { baseYield: 1000, defaultPrice: 50 };
    ...
    const predicted2026 = Math.round(baseUnitYield * acreageVal * weatherMultiplier * soilMultiplier * pestMultiplier);
  ...
  const get30DayHistoryData = () => {
    ...
    const uniqueDates = Array.from(new Set(cropPrices.map(p => p.date))).sort() as string[];

    const data = uniqueDates.map(date => {
      const dayPrices = cropPrices.filter(p => p.date === date);
      
      const kathmanduPrice = dayPrices.find(p => p.region === "Kathmandu")?.price_per_unit;
      const teraiPrice = dayPrices.find(p => p.region === "Terai")?.price_per_unit;
      const hillPrice = dayPrices.find(p => p.region === "Hill")?.price_per_unit;
      ...
      try {
        ...
        formattedDate = d.toLocaleDateString(language === "ne" ? "ne-NP" : "en-US", {
          month: "short",
          day: "numeric"
        });
      } catch (e) {
  ...
  const getPriceTrendAnalysis = () => {
    ...
    const key: "Average" | "Kathmandu" | "Terai" | "Hill" = historyRegion === "all" ? "Average" : historyRegion;
    ...
    if (percentChange > 2) {
      return {
        ...
        advice: t("Strong price appreciation over the past 30 days. Planting now is highly recommended as market demand remains robust, which points to high profit margins upon harvest.")
      };
      ...
      return {
        ...
        advice: t("Price is softening. It is recommended to optimize your cost of inputs, secure early buyer contracts, or consider staggered sowing to target later seasonal peak pricing.")
      };
    } else {
      return {
        direction: "stable",
        ...
        advice: t("Prices are holding stable with consistent market volume. Standard crop rotation schedules and regular planting yields are low-risk options under these conditions.")
      };
    }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1.5">
            <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 text-xs font-bold border border-emerald-500/30">
              ...
              <span>Farmer Dashboard &amp; Regional Operations (कृषि उपज व्यवस्थापन)</span>
            </div>
            <h2 className="text-xl md:text-2xl font-black tracking-tight text-white">
              Direct Farmer Produce &amp; Wholesale Operations
            </h2>
            <p className="text-xs text-emerald-100/80 max-w-2xl leading-relaxed">
              List regional crops, process B2B buyer contract orders, manage cooperative memberships, and monitor market price advisories in {user.district || "Kathmandu"} District.
            </p>
          </div>
      ...
      {currentUser.verified || currentUser.verificationStatus === 'verified' ? null : currentUser.verificationStatus === 'pending' ? (
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-800 mt-0.5 leading-relaxed">
                Your Citizenship Card (No: <strong>{currentUser.citizenshipNumber || "Uploaded"}</strong>) and National Identity Card scans are currently being audited by the Super Admin / Admin team. 
                You will be able to post product listings as soon as approval is granted.
              </p>
            </div>
          </div>
          <button
        ...
        <div className="bg-amber-500/10 border-2 border-amber-500/30 text-slate-900 rounded-2xl p-5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-start space-x-3.5">
            ...
            <div>
              <h4 className="font-bold text-slate-900 text-base font-display">Mandatory Verification Required for Selling Produce</h4>
              <p className="text-xs text-slate-600 mt-1 leading-relaxed">
                As per Nepal AgriTech safety regulations, farmers must upload clear scans of their <strong>Citizenship Card (नागरिकता)</strong> and <strong>National Identity Card (राष्ट्रिय परिचयपत्र)</strong>. 
                After approval by Super Admin / Admin, product selling and listing will be instantly activated.
              </p>
            </div>
          </div>
          <button
      ...
      {showWeatherBanner && (
        <div className={`border rounded-2xl p-5 shadow-sm transition-all duration-300 relative overflow-hidden ${
              ...
              <div className="space-y-1">
                <div className="flex flex-wrap items-center gap-2">
                  <h4 className="font-extrabold text-sm uppercase tracking-wide flex items-center">
                    {weatherAlertSeverity === "extreme" && t("🚨 RED ALERT: EXTREME DOWNPOUR & LANDSLIDE RISK")}
                    {weatherAlertSeverity === "severe" && t("⚠️ SEVERE WEATHER WARNING: WATERLOGGING RISK")}
                    {weatherAlertSeverity === "moderate" && t("ℹ️ ADVISORY: ACTIVE MONSOON SHOWERS")}
                    {weatherAlertSeverity === "normal" && t("☀️ WEATHER REPORT: ALL CLEAR")}
                  </h4>
                ...
                <p className="text-xs text-slate-700 leading-relaxed font-normal max-w-4xl">
                  {weatherAlertSeverity === "extreme" ? (
                    weatherAlertDistrict === "Dhading" ? (
                      <>
                        <strong>{t("Dhading Region:")}</strong> Extreme localized cloudburst forecasted (&gt;150mm rain in 24 hours) near the <strong>Trishuli River Basin</strong> and <strong>Galchhi / Malekhu</strong> sectors. Extremely high risk of highway-disrupting debris flows and landslides on the <strong>Prithvi Highway</strong>. Minimize slope traversal and secure hillside produce immediately.
                      </>
                    ) : weatherAlertDistrict === "Makwanpur" ? (
                      <>
                        <strong>{t("Makwanpur Region:")}</strong> Heavy orographic rainfall triggering emergency alert levels in the <strong>Kulekhani &amp; Sisneri Watersheds</strong>. Extreme mudslide and landslide warning for transport vehicles traversing the <strong>Tribhuvan Highway (Bhimfedi-Hetauda)</strong> and <strong>Kanti Lokpath</strong>. Secure storage barns.
                      </>
                    ) : (
                      <>
                        <strong>{t("Hilly Terraced Sector:")}</strong> Extreme heavy rain (&gt;120mm) predicted. Severe risk of flash mudslides on vulnerable terraced slopes. Clear run-off trenches to avoid direct topsoil erosion.
                      </>
                    )
                  ) : weatherAlertSeverity === "severe" ? (
                    <>
                      Continuous heavy monsoon showers are saturating terraced fields. Saturated ground increases risk of root decay. Trench out surplus pool water from tomato and potato beds immediately.
                    </>
                  ) : weatherAlertSeverity === "moderate" ? (
                    <>
                      Intermittent rainy periods with afternoon thunder. Excellent for rainwater reservoir replenishment, but delay nitrogen application to prevent fertilizer runoff wastage.
                    </>
                  ) : (
                    <>
                      Standard summer conditions. Current local climate parameters are safe. Ideal for harvesting, packaging, and open highway vehicle transport.
                    </>
                  )}
                </p>
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              <div className="flex items-center space-x-1">
                ...
                <select
                  ...
                  onChange={(e) => handleWeatherSeverityChange(e.target.value as any)}
                  className="bg-white border border-slate-200 text-slate-700 rounded-md py-0.5 px-1.5 text-[11px] font-semibold cursor-pointer outline-none focus:ring-1 focus:ring-emerald-500"
                >
          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 mt-0.5">
                Configure your channels for critical agricultural updates in {user.district} District
              </p>
            </div>
          </div>
          ...
          <div className="space-y-3 bg-slate-50 border border-slate-100 p-4 rounded-xl">
            <div className="flex items-start space-x-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500 leading-normal mt-0.5">
                  Get notified instantly when buyer demand posts surge or price forecasts spike for your local crops.
                </p>
              </div>
            </div>
            ...
            <div className="pt-2 flex flex-wrap gap-4 border-t border-slate-200/50">
              <label className="relative inline-flex items-center cursor-pointer select-none">
                ...
                <span className="ml-2 text-xs font-semibold text-slate-600">SMS Alerts</span>
              </label>
              ...
              <label className="relative inline-flex items-center cursor-pointer select-none">
                ...
                />
                <div className="w-8 h-4 bg-slate-200 rounded-full peer peer-focus:ring-1 peer-focus:ring-emerald-300 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:bg-emerald-600"></div>
                <span className="ml-2 text-xs font-semibold text-slate-600">In-App Alerts</span>
              </label>
            </div>
          </div>
          ...
          <div className="space-y-3 bg-slate-50 border border-slate-100 p-4 rounded-xl">
            <div className="flex items-start space-x-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500 leading-normal mt-0.5">
                  Receive critical weather hazard broadcasts (excessive rainfall, severe hailstorms, drought risks).
                </p>
              </div>
            </div>
      ...
      <div className="flex items-center space-x-2 border-b border-slate-200 dark:border-slate-800 pb-2 overflow-x-auto whitespace-nowrap">
        ...
        >
          ...
          <span>{t("Add crop")} ({listings.length})</span>
        </button>
        ...
        >
          ...
          <span>{t("Orders & Negotiations")} ({orders.filter(o => o.status !== "completed" && o.status !== "cancelled").length})</span>
        ...
        >
          ...
          <span>{t("Cooperatives")} ({coops.length})</span>
        </button>
        ...
        >
          ...
          <span>{t("Scan History")} ({scanLogs.length})</span>
        </button>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm h-fit">
            ...
            <form onSubmit={handleCreateListing} className="space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1.5 uppercase tracking-wider">Crop Item</label>
                <select
                  value={newCrop}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 mb-1.5 uppercase tracking-wider">Quantity (KG)</label>
                  <input
                    type="number"
                    placeholder="e.g. 500"
                    value={newQty}
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 mb-1.5 uppercase tracking-wider">Target (NRs/KG)</label>
                  <input
                    type="number"
                    placeholder="e.g. 65"
                    value={newPrice}
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              ...
              <button
                type="button"
                onClick={() => exportFarmerReportPDF(user, listings, orders, coops, coopMessages)}
            ...
            {listings.length === 0 ? (
              <div className="bg-slate-50 border border-slate-200 rounded-xl py-12 text-center text-slate-400 text-sm">
                You haven't listed any crops yet. Use the form to make your first offer.
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {listings.map((l) => (
                  <div key={l.id} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start">
                        <span className="font-bold text-slate-800 text-base font-display">{l.crop}</span>
                      ...
                      <div className="mt-4 grid grid-cols-2 gap-2 text-xs border-t border-b border-slate-100 py-3 my-3">
                        <div>
                          <span className="text-slate-400 block uppercase font-bold text-[9px] tracking-wider">Quantity</span>
                          <span className="font-bold text-slate-700 text-sm">{l.quantity} {l.unit}</span>
                        </div>
                        <div>
                          <span className="text-slate-400 block uppercase font-bold text-[9px] tracking-wider">Target Price</span>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between text-[11px] text-slate-400 mt-2 pt-2 border-t border-slate-100 gap-2">
                      <span className="font-mono">Listed {new Date(l.created_at).toLocaleDateString()}</span>
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-200 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Secure counter-offers and finalize transactions with real-time audit trails</p>
            </div>
            <button
              type="button"
              onClick={() => exportFarmerReportPDF(user, listings, orders, coops, coopMessages)}
                ...
                return (
                  <div key={o.id} className="transition duration-150 hover:bg-slate-50/20">
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex items-center space-x-2">
                          <span className="font-bold text-slate-800 text-sm font-display">{o.crop}</span>
                          <span className="text-slate-300">|</span>
                          <span className="text-xs text-slate-500">Buyer: <strong>{o.buyerName}</strong></span>
                        </div>
                        <div className="text-xs text-slate-400 flex flex-wrap gap-y-1 items-center space-x-3">
                          ...
                          <span>&bull;</span>
                          <span>Agreed Price: <strong className="text-slate-700">NRs. {o.agreed_price} / {o.unit}</strong></span>
                          <span>&bull;</span>
                          <span>Total: <strong className="text-emerald-700">NRs. {o.quantity * o.agreed_price}</strong></span>
                        </div>
                      </div>
                      ...
                      <div className="bg-slate-50 p-5 border-t border-slate-200 space-y-4">
                        ...
                        <div className="space-y-3 max-h-60 overflow-y-auto bg-white p-3 rounded-xl border border-slate-200 shadow-inner">
                          {orderHistory.length === 0 ? (
                            <div className="text-center text-slate-400 italic text-xs py-4">No counter proposals logged. Standard agreement.</div>
                          ) : (
                            orderHistory.map((n) => (
                              <div key={n.id} className={`flex flex-col p-2.5 rounded-lg border text-xs ${
                                ...
                                <div className="mt-1.5 font-bold text-slate-800 text-[10px] uppercase">
                                  Proposed Rate: <span className="text-emerald-700">NRs. {n.proposed_price} / {o.unit}</span>
                                </div>
                              </div>
                            ))
                          )}
                          ...
                          <div className="space-y-3 pt-2">
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                              ...
                              <div className="md:col-span-3">
                                <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Message to Buyer</label>
        ...
        <div className="space-y-4">
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">Connect with fellow agricultural producers, arrange shared transport to Kalimati, and secure volume wholesale discounts.</p>
            </div>
            ...
            {announcements.length === 0 ? (
              ...
              <div className="space-y-3.5 max-h-[300px] overflow-y-auto pr-1">
                {announcements.map((ann) => {
                  ...
                  return (
                    <div key={ann.id} className="bg-slate-50 border border-slate-150 rounded-xl p-4 space-y-2">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5">
                        <div className="flex items-center space-x-2">
                          <span className="font-bold text-slate-700 text-xs">{ann.cooperativeName}</span>
                      ...
                      <p className="text-slate-600 text-xs leading-relaxed font-normal whitespace-pre-wrap">{ann.content}</p>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {coops.length === 0 ? (
              ...
              coops.map((coop) => (
                <div key={coop.id} className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex flex-col justify-between">
                  <div>
                    <span className="text-[9px] uppercase font-bold tracking-wider text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-0.5 rounded-full">{coop.district} Hub</span>
                    <h4 className="font-bold font-display text-slate-800 text-base mt-2.5">{coop.name}</h4>
                    
                    <div className="mt-4 space-y-1.5 text-xs text-slate-600 border-t border-slate-100 pt-3">
                      <div>Contact Director: <strong>{coop.contact_person}</strong></div>
                      <div>Phone: <strong className="text-slate-800 font-mono">{coop.phone}</strong></div>
                    </div>
                  </div>

                  <div className="mt-4 pt-3 border-t border-slate-100 text-[11px] text-slate-400 flex justify-between items-center">
                    <span>{coop.farmerIds.includes(user.id) ? "You are a linked member" : "Open for membership"}</span>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
            ...
            {coopMessages.length === 0 ? (
              <p className="text-slate-400 text-xs italic py-2">
                No sent messages found. Use the 'Contact Cooperative' button on your active listings to start a dialogue.
              </p>
            ) : (
              <div className="space-y-3 max-h-80 overflow-y-auto pr-1">
                {coopMessages.map((msg) => (
                  <div key={msg.id} className="border border-slate-150 rounded-xl p-3.5 bg-slate-50/40 space-y-2">
                    <div className="flex justify-between items-start text-[11px]">
                      <div>
                        To: <strong className="text-slate-700">{msg.cooperativeName}</strong>
                      </div>
                    ...
                    <div className="text-[10px] flex items-center space-x-1">
                      ...
                      <span className="bg-indigo-50 text-indigo-700 font-bold px-1.5 py-0.5 rounded text-[9px] uppercase tracking-wider">{msg.crop}</span>
                    </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            ...
            <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">
              Estimate your upcoming harvest volume and potential market revenues based on historical district yields, regional climate trends, and farm management parameters.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4 h-fit">
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1.5 uppercase tracking-wider">Cultivated Area (Ropani)</label>
                <input
                  type="number"
                  min="0.1"
                  step="0.1"
                  ...
                  className="w-full bg-slate-50 border border-slate-200 hover:border-slate-300 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 focus:outline-none rounded-xl py-2 px-3 text-sm text-slate-700 transition"
                />
                <span className="text-[10px] text-slate-400 mt-1 block font-mono">1 Ropani &asymp; 508.7 sq. meters</span>
              </div>

              {/* Seasonal Rainfall Outlook */}
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1.5 uppercase tracking-wider">Seasonal Rainfall Pattern</label>
                <select
                  value={predRainfall}
                ...
                >
                  ...
                  <option value="optimal">Optimal / Season-appropriate Rainfall</option>
                  <option value="excessive">Excessive Monsoon / Heavy Storms</option>
                </select>
              </div>

              {/* Soil Management */}
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1.5 uppercase tracking-wider">Soil Nutrient Management</label>
                <select
                  value={predSoil}
                ...
                >
                  ...
                  <option value="organic">Standard Organic / Compost Only</option>
                  <option value="balanced">Balanced NPK / Optimal Fertilizer</option>
                </select>
              </div>

              {/* Pest & Disease Alert */}
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1.5 uppercase tracking-wider">Pest / Disease Threat level</label>
                <select
                  value={predPest}
                ...
                >
                  ...
                  <option value="moderate">Moderate Threat / Alert Active</option>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 shadow-sm relative overflow-hidden">
                  ...
                  <span className="text-[9px] uppercase font-bold tracking-wider text-slate-400">Predicted Yield</span>
                  <div className="text-xl font-bold font-display text-slate-800 mt-1.5 flex items-baseline space-x-1">
                    <span>{predictionStats.predicted2026.toLocaleString()}</span>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 shadow-sm relative overflow-hidden">
                  ...
                  <span className="text-[9px] uppercase font-bold tracking-wider text-slate-400">Est. Market Price</span>
                  <div className="text-xl font-bold font-display text-slate-800 mt-1.5 flex items-baseline space-x-1">
                ...
                <div className="bg-emerald-600 border border-emerald-700 rounded-xl p-4 shadow-sm text-white relative overflow-hidden">
                  <span className="text-[9px] uppercase font-bold tracking-wider text-emerald-200">Projected Revenue</span>
                  <div className="text-xl font-bold font-display mt-1.5 flex items-baseline space-x-1">
                    ...
                    <span>{predictionStats.estimatedRevenue.toLocaleString()}</span>
                  </div>
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                ...
                <div className="h-64 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={predictionStats.chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      ...
                      />
                      <Bar dataKey="Yield" fill="#94a3b8" radius={[8, 8, 0, 0]} maxBarSize={45}>
                        {predictionStats.chartData.map((entry, index) => {
                          const isPredicted = entry.name.includes("Predicted");
                          return <Cell key={`cell-${index}`} fill={isPredicted ? "#10b981" : "#cbd5e1"} />;
                        })}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 mt-0.5">
                      {t("Analyze day-to-day rate trends for")} <strong className="text-emerald-700">{t(predCrop)}</strong> {t("to time your sowing, transplanting, or harvest windows.")}
                    </p>
                  </div>

                  {/* Region Filter Buttons */}
                  <div className="flex items-center space-x-1 bg-slate-100 p-1 rounded-lg self-start sm:self-center">
                    {(["all", "Kathmandu", "Terai", "Hill"] as const).map((reg) => (
                      <button
                        key={reg}
                ...
                <div className="h-64 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={get30DayHistoryData()} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                      <XAxis dataKey="formattedDate" stroke="#94a3b8" fontSize={10} tickLine={false} />
                      <YAxis stroke="#94a3b8" fontSize={10} tickLine={false} label={{ value: t("NRs. / KG"), angle: -90, position: 'insideLeft', offset: 10, fill: '#94a3b8', fontSize: 10 }} />
                      <Tooltip
                        contentStyle={{ backgroundColor: "#1e293b", borderRadius: "12px", border: "none" }}
                        labelStyle={{ color: "#94a3b8", fontWeight: "bold", fontSize: "11px" }}
                        itemStyle={{ color: "#f8fafc", fontSize: "12px" }}
                      ...
                      {(historyRegion === "all" || historyRegion === "Kathmandu") && (
                        <Line
                          type="monotone"
                          dataKey="Kathmandu"
                          name={t("Kathmandu")}
                          ...
                          strokeDasharray={historyRegion === "all" ? "4 4" : undefined}
                        />
                      )}
                      
                      {(historyRegion === "all" || historyRegion === "Terai") && (
                        <Line
                          type="monotone"
                          dataKey="Terai"
                          name={t("Terai")}
                      ...
                      {(historyRegion === "all" || historyRegion === "Hill") && (
                        <Line
                          type="monotone"
                          dataKey="Hill"
                          name={t("Hill")}
                  ...
                  return (
                    <div className="bg-slate-50 rounded-xl p-4 border border-slate-150 space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`text-[10px] font-extrabold px-2 py-0.5 rounded-full uppercase tracking-wider ${analysis.badge}`}>
                          {analysis.desc} {analysis.percentChange !== 0 && `(${analysis.percentChange > 0 ? '+' : ''}${analysis.percentChange}%)`}
                        </span>
                      </div>
                      ...
                      <div className="pt-1.5 flex items-center space-x-2 text-[10px] text-slate-400">
                        ...
                        <span>{t("Analyzing Kalimati, Tokha, & Terai regional wholesale feed...")}</span>
                      </div>
                    </div>
                  );
                })()}
              ...
              <div className="bg-emerald-50/50 border border-emerald-100 rounded-xl p-5 shadow-sm space-y-3.5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {getRecommendation().map((rec, idx) => (
                    <div key={idx} className="bg-white border border-emerald-100/80 rounded-xl p-4 shadow-sm space-y-1">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">
                Personalized 7-day agricultural weather forecast for <strong className="text-slate-700">{user.district || "Kathmandu"} District</strong>. Select your crop below to see optimal harvesting dates.
              </p>
            </div>
            
            {/* Crop Selector */}
            <div className="flex items-center space-x-2 shrink-0 bg-slate-50 border border-slate-200 rounded-xl p-2">
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider pl-1">Crop:</span>
              <select
                value={climateCrop}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
            {generateForecast(user.district || "Kathmandu").map((day, idx) => {
              ...
              return (
                <div key={idx} className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm flex flex-col justify-between hover:border-slate-300 transition relative">
                  {idx === 0 && (
                  ...
                  <div className="space-y-2">
                    <div className="text-center">
                      ...
                      <p className="text-[10px] text-slate-400 font-mono">{day.dateStr}</p>
                    </div>

                    <div className="flex justify-center py-2">
                      {day.iconType === "sun" && <Sun className="w-8 h-8 text-amber-500" />}
                      {day.iconType === "sun-cloud" && <CloudSun className="w-8 h-8 text-amber-500" />}
                      {day.iconType === "rain" && <CloudRain className="w-8 h-8 text-blue-400" />}
                      {day.iconType === "storm" && <CloudLightning className="w-8 h-8 text-purple-500" />}
                      {day.iconType === "cloud" && <Cloud className="w-8 h-8 text-slate-400" />}
                    </div>

                    <div className="text-center">
                      <p className="text-xs font-semibold text-slate-700 leading-tight">{day.condition}</p>
                    ...
                    <div className="border-t border-slate-100 pt-2 space-y-1 text-[10px] font-semibold text-slate-500 font-mono">
                      <div className="flex justify-between items-center">
                        <span className="flex items-center text-[9px] uppercase font-bold text-slate-400">
                          <Droplets className="w-3 h-3 text-blue-400 mr-1 shrink-0" /> Rain %
                        </span>
                        <span>{day.rainProb}%</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="flex items-center text-[9px] uppercase font-bold text-slate-400">
                          <Droplets className="w-3 h-3 text-slate-400 mr-1 shrink-0" /> Humid
                        </span>
                        <span>{day.humidity}%</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="flex items-center text-[9px] uppercase font-bold text-slate-400">
                          <Wind className="w-3 h-3 text-slate-400 mr-1 shrink-0" /> Wind
                        </span>
                  ...
                  <div className={`mt-3 p-2 rounded-lg border text-[10px] ${suitability.color}`}>
                    <div className="flex items-center justify-between font-bold uppercase tracking-wider mb-1">
                      <span>Harvest:</span>
                      <span className={`px-1.5 py-0.5 rounded text-[8px] ${suitability.badge}`}>
                        {suitability.rating}
                      </span>
                    </div>
                    <p className="leading-snug text-slate-600 font-normal">{suitability.desc}</p>
                  </div>
                </div>
              );
            })}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
              ...
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart
                    data={generateForecast(user.district || "Kathmandu").map(d => ({
                      name: d.dayName,
                  ...
                  >
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                    <XAxis dataKey="name" stroke="#94a3b8" fontSize={11} tickLine={false} />
                    <YAxis yAxisId="left" stroke="#e2e8f0" label={{ value: 'Temp (°C)', angle: -90, position: 'insideLeft', offset: 10, fill: '#64748b', fontSize: 10 }} />
                    <YAxis yAxisId="right" orientation="right" stroke="#e2e8f0" label={{ value: 'Rain (%)', angle: 90, position: 'insideRight', offset: 10, fill: '#64748b', fontSize: 10 }} />
                    <Tooltip
                      contentStyle={{ backgroundColor: "#1e293b", borderRadius: "12px", border: "none" }}
                      labelStyle={{ color: "#94a3b8", fontWeight: "bold", fontSize: "11px" }}
                      itemStyle={{ color: "#f8fafc", fontSize: "12px" }}
                    />
                    <Legend wrapperStyle={{ fontSize: '11px', fontWeight: 600, color: '#475569' }} />
                    <Line yAxisId="left" type="monotone" dataKey="Max Temp (°C)" stroke="#f97316" strokeWidth={2.5} activeDot={{ r: 6 }} />
                    <Line yAxisId="right" type="monotone" dataKey="Rain Prob (%)" stroke="#3b82f6" strokeWidth={2.5} activeDot={{ r: 6 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex flex-col justify-between">
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  {(() => {
                    const forecast = generateForecast(user.district || "Kathmandu");
                      ...
                      <>
                        <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 space-y-2">
                          ...
                          <p className="text-[11px] text-slate-600 leading-relaxed">
                            {dryDays.length > 0 ? (
                              <span>
                                The best window to harvest <strong className="text-slate-800">{climateCrop}</strong> is on{" "}
                                <strong className="text-emerald-700">{dryDays.map(d => d.dayName).join(", ")}</strong> due to low rain risks ({dryDays[0]?.rainProb}%).
                              </span>
                            ) : (
                              <span>
                                Persistent wet patterns forecasted. Delay bulk harvest of <strong className="text-slate-800">{climateCrop}</strong> if possible. If harvesting is urgent, focus on protective covers.
                              </span>
                            )}
                          </p>
                        </div>
                        ...
                        <div className="space-y-2.5 pt-1">
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Agronomic Task List</p>
                          
                          {wetDays.length > 0 && (
                            <div className="flex items-start space-x-2 text-[11px] text-slate-600">
                              <AlertCircle className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
                              <div>
                                <strong className="text-slate-800">Moisture Protection:</strong> Heavy rain is forecasted on {wetDays.map(d => d.dayName).slice(0, 2).join(" & ")}. Clean cooperative storage bays or lay out dry tarps in advance.
                              </div>
                            </div>
                          )}
                          ...
                          {climateCrop === "Potato (Alu)" && (
                            <div className="flex items-start space-x-2 text-[11px] text-slate-600">
                              ...
                              <div>
                                <strong className="text-slate-800">De-halming (Vines cutting):</strong> Cut potato vines 10-12 days before the dry weather window to thicken tuber skins for bruising prevention.
                              </div>
                            </div>
                          )}
                          ...
                          {climateCrop === "Tomato (Golbheda)" && (
                            <div className="flex items-start space-x-2 text-[11px] text-slate-600">
                              <Check className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
                              <div>
                                <strong className="text-slate-800">Airflow Management:</strong> Prune lower leaves now to reduce humidity build-up and minimize late blight spread before showers start.
                              </div>
                            </div>
                          )}
                          ...
                          {climateCrop === "Onion (Pyaj)" && (
                            <div className="flex items-start space-x-2 text-[11px] text-slate-600">
                              <Check className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
                              <div>
                                <strong className="text-slate-800">Dry Curing:</strong> Ensure harvested bulbs are not left on moist soil. Cure them on raised wire racks under shade to prevent sunscald.
                              </div>
                            </div>
                          )}
                          ...
                          <div className="flex items-start space-x-2 text-[11px] text-slate-600">
                            <Check className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
                            <div>
                              <strong className="text-slate-800">Cooperative Logistics:</strong> Coordinate transportation via your district cooperative to move bulk stock on clear days.
                            </div>
                          </div>
                        </div>
                      </>
        ...
        <div className="space-y-6">
              ...
              <div className="flex items-center space-x-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 leading-relaxed">
                    Gemini Climatology Engine analyzes seasonal weather patterns, temperature anomalies, and historic wholesales to recommend the best crops for <strong className="text-slate-700">{user.district || "Kathmandu"}</strong>.
                  </p>
                </div>
              </div>
            </div>
          ...
          {advisorError && (
            <div className="bg-red-50 border border-red-200 text-red-800 text-xs rounded-xl p-4 flex items-start space-x-2.5 shadow-sm">
              ...
              <div>
                <strong className="block mb-0.5 font-bold">Climate Analysis Disrupted</strong>
                <p>{advisorError}</p>
                <button
                  onClick={handleGenerateAIAdvisor}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  {advisorResult.recommendedCrops.map((crop, idx) => (
                    <div key={idx} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition duration-150 flex flex-col justify-between space-y-5">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="text-[11px] font-bold text-slate-500 bg-slate-50 border border-slate-150 rounded-lg py-1 px-2.5 inline-block">
                          Variety: <span className="text-purple-700 font-extrabold">{crop.variety}</span>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3.5 border-y border-slate-100 py-4 font-semibold text-slate-600 text-xs">
                        <div className="space-y-1">
                          <span className="text-[9px] uppercase font-bold text-slate-400 tracking-wider flex items-center">
                            <TrendingUp className="w-3.5 h-3.5 text-emerald-500 mr-1" /> Yield Potential
                          </span>
                          <span className="font-bold text-slate-700 font-mono">{crop.yieldPotential}</span>
                        </div>

                        <div className="space-y-1">
                          <span className="text-[9px] uppercase font-bold text-slate-400 tracking-wider flex items-center">
                            <Droplets className="w-3.5 h-3.5 text-blue-400 mr-1" /> Irrigation
                          </span>
                          <span className="font-bold text-slate-700">{crop.irrigationRequirement}</span>
                        </div>

                        <div className="space-y-1 col-span-2">
                          <span className="text-[9px] uppercase font-bold text-slate-400 tracking-wider flex items-center">
                            <DollarSign className="w-3.5 h-3.5 text-emerald-500 mr-0.5" /> Market Trend &amp; Demand
                          </span>
                          <span className="text-[11px] font-medium text-slate-600 leading-relaxed block">{crop.marketDemandTrend}</span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <ul className="space-y-2">
                          {crop.agronomicTips.map((tip, tIdx) => (
                            <li key={tIdx} className="text-[11px] text-slate-600 leading-relaxed flex items-start space-x-1.5 font-medium">
              ...
              <div className="bg-purple-50/50 border border-purple-150 rounded-2xl p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {advisorResult.climateAdaptationAdvice.map((advice, idx) => (
                    <div key={idx} className="bg-white border border-purple-100 p-4 rounded-xl flex items-start space-x-3.5 shadow-xs">
            ...
            <form onSubmit={handleCreateSoilLog} className="space-y-4">
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-1">
                  Specify which crop batch or partition of your land this applies to.
                </p>
              </div>
                ...
                <div className="flex flex-wrap gap-1">
                  {soilLogType === "fertilizer" ? (
                    <>
                      <button
                        type="button"
                        onClick={() => setSoilDetails("Applied 50kg organic farmyard manure (Compost) to boost micronutrient soil health.")}
                      ...
                      <button
                        type="button"
                        onClick={() => setSoilDetails("Applied chemical booster: 15kg Urea (Nitrogen) & 10kg Potash.")}
                        className="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-600 px-2 py-1 rounded transition text-left"
                      >
                        + N-K Booster
                      </button>
                      <button
                        type="button"
                        onClick={() => setSoilDetails("Added 20kg DAP (Diammonium Phosphate) before seeding to strengthen roots.")}
                        className="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-600 px-2 py-1 rounded transition text-left"
                      >
                        + DAP Root Care
                      </button>
                    ...
                    <>
                      <button
                        type="button"
                        onClick={() => setSoilDetails("Soil pH: 6.5 (Optimal). N-P-K values are balanced. Standard maintenance recommended.")}
                        className="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-600 px-2 py-1 rounded transition text-left"
                      >
                        + Balanced pH
                      </button>
                      <button
                        type="button"
                        onClick={() => setSoilDetails("Soil pH: 5.5 (Acidic). Nitrogen is low. Recommended adding agricultural lime and nitrogen-rich manure.")}
                        className="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-600 px-2 py-1 rounded transition text-left"
                      >
                        + Acidic + Low N
                      </button>
                      <button
                        type="button"
                        onClick={() => setSoilDetails("Soil pH: 7.2 (Slightly Alkaline). Phosphorus is low. Recommended applying bone meal or acidifying fertilizers.")}
                        className="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-600 px-2 py-1 rounded transition text-left"
                      >
                        + Alkaline + Low P
                      </button>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 pb-3 mb-4 space-y-2 sm:space-y-0">
                <h3 className="font-bold font-display text-slate-800 text-sm tracking-wide uppercase flex items-center space-x-1.5">
                  ...
                  <span>Soil &amp; Fertilizer Application Logs ({soilLogs.length})</span>
                </h3>
              ...
              {soilLogs.length === 0 ? (
                <div className="text-center py-12 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                  ...
                  <p className="text-slate-600 font-medium text-xs">No records logged yet</p>
                  <p className="text-slate-400 text-[11px] max-w-xs mx-auto mt-1 leading-normal">
                    Track fertilizer dosages and soil test results here. Use the form to submit your first soil or nutrient activity.
                  </p>
                </div>
              ) : (
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-slate-50 border border-slate-100 p-3.5 rounded-xl space-y-1.5">
                  ...
                  <ul className="text-[11px] text-slate-600 leading-relaxed list-disc list-inside space-y-1">
                    <li>Optimal Soil pH: <strong className="text-slate-800">6.0 - 6.8</strong></li>
                    <li>Prefers well-draining loam or sandy loam with lots of compost.</li>
                    <li>Needs Phosphorus for initial root set, followed by Potassium during fruiting.</li>
                  </ul>
                </div>
                ...
                <div className="bg-slate-50 border border-slate-100 p-3.5 rounded-xl space-y-1.5">
                  ...
                  <ul className="text-[11px] text-slate-600 leading-relaxed list-disc list-inside space-y-1">
                    <li>Optimal Soil pH: <strong className="text-slate-800">4.8 - 6.0</strong> (Acidic prevents common potato scab).</li>
                    <li>Demands heavy Potassium for tuber size and starch synthesis.</li>
                    <li>Avoid heavy fresh manure right before planting to prevent rot.</li>
                  </ul>
                </div>
              </div>
              ...
              <div className="mt-4 bg-emerald-50/50 border border-emerald-100 rounded-xl p-3.5">
                ...
                <p className="text-[10px] text-slate-600 leading-relaxed mt-1">
                  <strong>N (Nitrogen/Urea)</strong>: Drives lush, green leaf growth. Deficient plants look pale yellow. <br />
                  <strong>P (Phosphorus/DAP)</strong>: Powers healthy root structures and flowering. Deficient leaves look dark purple. <br />
                  <strong>K (Potassium/Potash)</strong>: Strengthens immunity against disease and enhances crop size/quality. Deficient leaf edges look brown.
                </p>
              </div>
            </div>
      ...
      {activeTab === "forecasting" && (() => {
        const userHarvests = harvestRecords.filter(h => h.farmerId === user.id);
        
        // Chronological order for chart
        const seasonOrder = ["Spring 2024", "Summer 2024", "Autumn 2024", "Winter 2024", "Spring 2025", "Summer 2025", "Autumn 2025", "Winter 2025"];
        ...
        const seasonsWithData = Array.from(new Set(sortedUserHarvests.map(h => h.season)));
        seasonsWithData.sort((a, b) => seasonOrder.indexOf(a) - seasonOrder.indexOf(b));

        const chartData = seasonsWithData.map(season => {
          const seasonRecords = sortedUserHarvests.filter(h => h.season === season);
          const dataPoint: any = { name: season };
          const cropsList = ["Potato (Alu)", "Tomato (Golbheda)", "Cauliflower (Kauli)", "Ginger (Aduwa)", "Onion (Pyaj)"];
          cropsList.forEach(crop => {
            const cropRecord = seasonRecords.find(r => r.crop === crop);
            if (cropRecord) {
              dataPoint[crop] = Math.round(cropRecord.yieldQuantity / cropRecord.acreage);
            }
          });
          return dataPoint;
        });
        ...
        const recommendations = cropsList.map(cropName => {
          ...
          const userCropRecords = userHarvests.filter(h => h.crop === cropName);
          const totalUserYield = userCropRecords.reduce((sum, r) => sum + r.yieldQuantity, 0);
          const totalUserAcreage = userCropRecords.reduce((sum, r) => sum + r.acreage, 0);
          const userAvgYieldPerRopani = totalUserAcreage > 0 ? (totalUserYield / totalUserAcreage) : 0;

          // Regional yield records
          const allCropRecords = harvestRecords.filter(h => h.crop === cropName);
          const totalAllYield = allCropRecords.reduce((sum, r) => sum + r.yieldQuantity, 0);
          const totalAllAcreage = allCropRecords.reduce((sum, r) => sum + r.acreage, 0);
          const regionalAvgYieldPerRopani = totalAllAcreage > 0 ? (totalAllYield / totalAllAcreage) : (CROP_BASE_STATS[cropName]?.baseYield || 1000);

          // Best user harvest season for this crop
          const bestHarvest = userCropRecords.length > 0 
            ? userCropRecords.reduce((prev, curr) => (curr.yieldQuantity / curr.acreage) > (prev.yieldQuantity / prev.acreage) ? curr : prev)
            : null;

          const baseYieldPerRopani = userAvgYieldPerRopani > 0 ? userAvgYieldPerRopani : regionalAvgYieldPerRopani;
          ...
          const forecastedYieldPerRopani = baseYieldPerRopani * weatherMult * soilMult;
          const forecastedYield = forecastedYieldPerRopani * Number(simAcreage);

          // Current Market Price
          const cropPrices = marketPrices.filter(p => p.crop === cropName);
          const currentPrice = cropPrices.length > 0 ? cropPrices[0].price_per_unit : (CROP_BASE_STATS[cropName]?.defaultPrice || 50);
          const expectedRevenue = forecastedYield * currentPrice;

          // Advice summary
          const adviceList = [];
          if (simWeather === 'dry') {
            adviceList.push("Requires protective straw mulching to preserve ground moisture.");
          } else if (simWeather === 'excessive') {
            adviceList.push("Build deep runoff trenches and raised planting beds (25-30cm) to secure high drainage.");
          } else {
            adviceList.push("Ideal season condition. Standard planting depth with moderate watering schedules is sufficient.");
          }

          if (simSoil === 'poor') {
            adviceList.push("Add heavy organic manure & root booster DAP fertilizer prior to sowing seeds.");
          } else if (simSoil === 'organic') {
            adviceList.push("Incorporate bio-fertilizers like vermicompost and liquid organic tea sprays.");
          } else {
            adviceList.push("Balanced fertilizer ready. Utilize balanced NPK 19-19-19 chemical nutrient application.");
          }
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-emerald-800 to-teal-700 rounded-2xl p-6 text-white shadow-md">
              <div className="max-w-4xl space-y-2">
                ...
                <p className="text-xs text-emerald-100/90 leading-relaxed max-w-3xl">
                  Analyze multi-season harvest trends across your specific acreage in {user.district}. Log past harvests below to train the recommendation algorithm on your farm's unique performance baseline.
                </p>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-4 gap-3">
                <div className="flex items-center space-x-2.5">
                  ...
                  <div>
                    <h3 className="font-bold text-sm text-slate-800">AI-Powered Harvest &amp; Planting Forecaster</h3>
                    <p className="text-xs text-slate-500">
                      Gemini deep intelligence leverages historical {user.district} district data and live soil metrics to predict optimal sowing schedules.
                    </p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end bg-slate-50/50 p-4 rounded-xl border border-slate-150">
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5">Target Crop</label>
                  <select
                    value={aiCrop}
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5">Acreage (Ropani)</label>
                  <input
                    type="number"
                    min="0.1"
                    step="0.1"
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5">Current Soil Condition</label>
                  <select
                    value={aiSoil}
                  ...
                  >
                    ...
                    <option value="balanced">Balanced (Optimal pH &amp; NPK)</option>
                    <option value="organic">Organic Rich (Heavy composted)</option>
                  </select>
                </div>
              ...
              {aiError && (
                <div className="bg-red-50 border border-red-200 text-red-800 text-xs rounded-xl p-4 flex items-start space-x-2">
                  ...
                  <div>
                    <span className="font-bold block mb-0.5">AI Engine Error</span>
                    <span>{aiError}</span>
                  </div>
                </div>
              )}
                  ...
                  <div className="p-5 space-y-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="bg-white border border-slate-150 rounded-xl p-4 space-y-1">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Estimated Yield</span>
                        ...
                        <span className="text-[10px] text-slate-500 block">Calculated across {aiAcreage} Ropani</span>
                      </div>

                      <div className="bg-white border border-slate-150 rounded-xl p-4 space-y-1">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Recommended Cycle</span>
                        <div className="text-base font-extrabold text-purple-700 uppercase tracking-tight truncate mt-0.5">
                          {aiResult.plantingCycle}
                        </div>
                        <span className="text-[10px] text-slate-500 block">Selected for {user.district} micro-climate</span>
                      </div>

                      <div className="bg-white border border-slate-150 rounded-xl p-4 space-y-1">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Optimal Timelines</span>
                        <div className="text-xs font-bold text-slate-700 mt-1 space-y-0.5">
                          <div className="flex justify-between">
                            <span className="text-slate-400 font-medium">Sowing:</span>
                            <span className="font-mono">{aiResult.optimalPlantingDate}</span>
                          </div>
                          <div className="flex justify-between border-t border-slate-100 pt-0.5">
                            <span className="text-slate-400 font-medium">Harvest:</span>
                            <span className="font-mono">{aiResult.optimalHarvestDate}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2.5">
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">AI Sequential Implementation Steps:</span>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {aiResult.actionableSteps.map((step, idx) => (
                          <div key={idx} className="bg-white border border-slate-200 p-3.5 rounded-xl flex items-start space-x-2.5 shadow-xs">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm h-fit">
                ...
                <form onSubmit={handleCreateHarvestRecord} className="space-y-4">
                  {/* Select Crop */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">
                      Crop Type
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setHarvestWeather(e.target.value as any)}
                    ...
                    <div>
                      ...
                      <select
                        value={harvestSoil}
                        onChange={(e) => setHarvestSoil(e.target.value as any)}
                        className="w-full text-xs border border-slate-200 rounded-lg py-1.5 px-2 focus:ring-1 focus:ring-emerald-500"
                      >
                  ...
                  <button
                    ...
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold py-2.5 px-4 rounded-lg transition duration-150 shadow-sm disabled:opacity-50"
                  >
                    {isSubmittingHarvest ? "Recording..." : "Record Past Harvest"}
                  </button>
                </form>
              </div>
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5">Target Season</label>
                      <select
                        value={simSeason}
                    ...
                    {/* Sim Acreage */}
                    <div>
                      <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5">Simulated Land (Ropani)</label>
                      <input
                        type="number"
                        min="0.1"
                        step="0.1"
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5">Expected Weather</label>
                      <select
                        value={simWeather}
                    ...
                    {/* Sim Expected Soil */}
                    <div>
                      <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5">Soil Quality Plan</label>
                      <select
                        value={simSoil}
                        onChange={(e) => setSimSoil(e.target.value as any)}
                        className="w-full text-xs bg-slate-50 border border-slate-200 rounded-lg py-2 px-2.5 font-medium text-slate-700"
                      >
                        <option value="poor">Poor / Minimal Fertilizer</option>
                        <option value="organic">Organic / Heavy Compost</option>
                        <option value="balanced">Balanced NPK / Optimal</option>
                      </select>
                    </div>
                  </div>
                </div>
                        ...
                        >
                          <div className="flex items-start justify-between">
                            ...
                            <div className="text-right">
                              <span className="text-[10px] text-slate-400 uppercase tracking-wider block font-semibold">Expected Revenue</span>
                          ...
                          <div className="grid grid-cols-2 gap-2 mt-4 py-2 border-t border-b border-slate-100 text-xs">
                            <div>
                              <span className="text-[10px] text-slate-400 block uppercase font-medium">Estimated Yield</span>
                              <span className="font-bold text-slate-700 font-mono">{rec.forecastedYield} KG</span>
                            </div>
                            <div>
                              <span className="text-[10px] text-slate-400 block uppercase font-medium">Efficiency Baseline</span>
                              <span className="font-semibold text-slate-600 font-mono">{rec.forecastedYieldPerRopani} KG/Ropani</span>
                            </div>
                          </div>
                          ...
                          <div className="mt-3.5 space-y-2">
                            ...
                            {rec.isBasedOnPersonalHistory ? (
                              <div className="bg-emerald-50/70 border border-emerald-100 text-[10px] text-emerald-800 p-2 rounded-lg leading-relaxed flex items-start space-x-1.5">
                                ...
                                <span>
                                  Based on your own farm history. Your best record: <strong>{rec.bestHarvest ? Math.round(rec.bestHarvest.yieldQuantity / rec.bestHarvest.acreage) : 0} KG/Ropani</strong> in {rec.bestHarvest?.season}.
                                </span>
                              </div>
                            ) : (
                              <div className="bg-slate-50 border border-slate-200 text-[10px] text-slate-500 p-2 rounded-lg leading-relaxed flex items-start space-x-1.5">
                                <span className="text-xs shrink-0">📋</span>
                                <span>
                                  No personal history logged yet for this crop. Forecasting initialized with regional district averages of <strong>{rec.baseYieldPerRopani} KG/Ropani</strong>.
                                </span>
                              </div>
                            )}
                            ...
                            <div className="space-y-1.5 bg-slate-50/50 p-3 rounded-lg border border-slate-100">
                              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide block">Actionable Planting Guides:</span>
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                  <div className="border-b border-slate-100 pb-3 mb-4">
                    <h3 className="font-bold font-display text-slate-800 text-sm tracking-wide uppercase flex items-center space-x-1.5">
                      ...
                      <span>Historical Yield Performance Trends (KG / Ropani)</span>
                    </h3>
                    <p className="text-[11px] text-slate-500 mt-0.5 leading-normal">
                      Compare seasonal soil efficiency milestones. More recorded harvests refine this line chart naturally.
                    </p>
                  </div>
                  ...
                  {userHarvests.length === 0 ? (
                    <div className="text-center py-12 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                      <Sprout className="w-8 h-8 text-slate-300 mx-auto mb-2.5 animate-bounce" />
                      <p className="text-slate-600 font-medium text-xs">No historical records logged yet</p>
                      <p className="text-slate-400 text-[11px] max-w-xs mx-auto mt-1 leading-normal">
                        Submit a past harvest using the form to populate the efficiency line charts instantly.
                      </p>
                    </div>
                  ) : (
                    <div className="h-[280px] w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                          ...
                          <Tooltip contentStyle={{ fontSize: 11, borderRadius: 8, borderColor: "#cbd5e1" }} />
                          <Legend wrapperStyle={{ fontSize: 11 }} />
                          <Line type="monotone" dataKey="Tomato (Golbheda)" stroke="#ef4444" strokeWidth={2.5} dot={{ r: 4 }} connectNulls />
                          <Line type="monotone" dataKey="Potato (Alu)" stroke="#3b82f6" strokeWidth={2.5} dot={{ r: 4 }} connectNulls />
                          <Line type="monotone" dataKey="Cauliflower (Kauli)" stroke="#10b981" strokeWidth={2.5} dot={{ r: 4 }} connectNulls />
                          <Line type="monotone" dataKey="Ginger (Aduwa)" stroke="#f59e0b" strokeWidth={2.5} dot={{ r: 4 }} connectNulls />
                          <Line type="monotone" dataKey="Onion (Pyaj)" stroke="#8b5cf6" strokeWidth={2.5} dot={{ r: 4 }} connectNulls />
                        </LineChart>
                      </ResponsiveContainer>
                    </div>
                  )}
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
                    <h3 className="font-bold font-display text-slate-800 text-sm tracking-wide uppercase flex items-center space-x-1.5">
                      <History className="w-4 h-4 text-emerald-600" />
                      <span>My Logged Harvests Timeline ({userHarvests.length})</span>
                    </h3>
                  </div>

                  {userHarvests.length === 0 ? (
                    <div className="text-center py-6 text-slate-400 text-xs">
                      No past harvest logs stored. Use the left panel form to build your history.
                    </div>
                  ) : (
                        ...
                        >
                          <div className="space-y-1">
                            <div className="flex items-center space-x-2">
                              ...
                              <span className="text-slate-300">&bull;</span>
                              <span className="font-semibold text-slate-500 font-mono">{rec.season}</span>
                            </div>
                            <p className="text-slate-600 leading-normal">
                              Land Area: <strong>{rec.acreage} Ropani</strong> &bull; Total Yield: <strong>{rec.yieldQuantity} KG</strong> (<strong className="text-emerald-700 font-mono">{Math.round(rec.yieldQuantity / rec.acreage)} KG/Ropani</strong> efficiency)
                            </p>
                            <p className="text-[10px] text-slate-400">
                              Soil: <strong className="capitalize">{rec.soilCondition}</strong> &bull; Weather: <strong className="capitalize">{rec.weatherCondition}</strong> &bull; Nutrients: <strong>{rec.fertilizerUsed}</strong>
                            </p>
                          </div>
        ...
        <div className="space-y-6">
            ...
            <div className="lg:col-span-5 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
              <div className="border-b border-slate-100 pb-3">
                ...
                <p className="text-[11px] text-slate-500 mt-1 leading-normal">
                  Receive an automated alert when a crop's market price violates your target threshold.
                </p>
              </div>
              ...
              <form onSubmit={handleCreateAlert} className="space-y-4">
                ...
                >
                  ...
                  <span>{isSubmittingAlert ? "Registering..." : "Enable Alert Watch"}</span>
                </button>
              </form>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3 pt-3">
                <div className="flex items-center space-x-2 text-slate-800">
                  ...
                  <span className="font-bold text-xs uppercase tracking-wider">Interactive Test Bed</span>
                </div>
                <p className="text-[10px] text-slate-500 leading-normal">
                  Simulate a live market price update for your registered crop alert above to test instant trigger notifications:
                </p>

                <div className="flex gap-2 items-center">
                  <div className="flex-1">
                    <span className="text-[9px] font-bold text-slate-400 block uppercase mb-1">Simulated Price</span>
                    <input
                      type="number"
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                  <h3 className="font-bold font-display text-slate-800 text-sm tracking-wide uppercase flex items-center space-x-2">
                    ...
                    <span>My Registered Alert Watches ({priceAlerts.length})</span>
                  </h3>
                </div>

                {isFetchingAlerts ? (
                  <p className="text-center py-6 text-xs text-slate-400">Loading your watches...</p>
                ) : priceAlerts.length === 0 ? (
                  <div className="text-center py-10 bg-slate-50 border border-dashed border-slate-200 rounded-xl">
                    <Bell className="w-8 h-8 text-slate-300 mx-auto mb-2" />
                    <p className="text-slate-500 font-semibold text-xs">No active price alert watches</p>
                    <p className="text-slate-400 text-[10px] max-w-xs mx-auto mt-1 leading-relaxed">
                      Farmers register watches to track price fluctuations. Create one on the left to start!
                    </p>
                  </div>
                ) : (
                    ...
                    {priceAlerts.map((alert) => (
                      <div
                        ...
                        className={`border rounded-xl p-3.5 flex items-center justify-between transition ${
                          alert.isActive ? "bg-slate-50 border-slate-200/80" : "bg-slate-50/50 border-slate-100 opacity-60"
                        }`}
                      >
                        <div className="space-y-1">
                          <div className="flex items-center space-x-2">
                            <span className="font-bold text-slate-800 text-xs">{alert.crop}</span>
                            <span className="text-slate-300">&bull;</span>
                            <span className="text-[10px] text-slate-400 font-mono capitalize">
                              Region: {alert.region === "all" ? "Any" : alert.region}
                            </span>
                          </div>
                          
                          <div className="flex items-center space-x-1.5 text-xs">
                            <span className="text-slate-500">Alert if price is</span>
                            <span className={`font-bold uppercase text-[10px] px-2 py-0.25 rounded-md ${
                              alert.criteria === "above" 
                                ? "bg-emerald-100 text-emerald-800" 
                                : "bg-blue-100 text-blue-800"
                            }`}>
                              {alert.criteria === "above" ? "≥ Above" : "≤ Below"}
                            </span>
                            <span className="font-extrabold text-slate-700 font-mono">NRs. {alert.priceThreshold}</span>
                          </div>
                          <span className="text-[9px] text-slate-400 font-medium block">
                            Registered on {new Date(alert.created_at).toLocaleDateString()}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <label className="relative inline-flex items-center cursor-pointer select-none">
                            <input
                              ...
                              onChange={() => handleToggleAlertActive(alert.id, alert.isActive)}
                              className="sr-only peer"
                            />
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-100 pb-3 gap-3">
              <div>
                <h3 className="font-bold font-display text-slate-800 text-sm tracking-wide uppercase flex items-center space-x-2">
                  ...
                  <span>Triggered In-App Alerts Logs ({notifications.length})</span>
                </h3>
                <p className="text-[11px] text-slate-500 mt-1 leading-normal">
                  History of verified market fluctuations matching your alert target configurations.
                </p>
              </div>
            ...
            {isFetchingNotifications ? (
              <p className="text-center py-6 text-xs text-slate-400">Loading your history log...</p>
            ) : notifications.length === 0 ? (
              <div className="text-center py-12 bg-slate-50 border border-dashed border-slate-200 rounded-2xl">
                <BellRing className="w-8 h-8 text-slate-300 mx-auto mb-2.5" />
                <p className="text-slate-600 font-bold text-xs">No notifications logged yet</p>
                <p className="text-slate-400 text-[10px] max-w-xs mx-auto mt-1 leading-relaxed">
                  Notifications trigger automatically when an administrator logs new prices or during test bed simulations.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[380px] overflow-y-auto pr-1">
                {notifications.map((notif) => (
                  ...
                  >
                    {!notif.isRead && (
                      <span className="absolute top-3 right-3 w-2 h-2 bg-rose-500 rounded-full animate-ping" title="Unread"></span>
                    )}
                    ...
                    <div className="mt-4 pt-3 border-t border-slate-100/80 flex justify-between items-center text-[11px]">
                      <div className="flex items-center space-x-2 font-semibold">
                        ...
                        <span className="font-bold text-slate-800 font-mono">NRs. {notif.currentPrice}</span>
                        <span className="text-slate-300">/</span>
                        <span className="text-slate-400">Threshold:</span>
                        <span className="font-bold text-slate-700 font-mono">NRs. {notif.threshold}</span>
                      </div>
      ...
      {activeTab === "calculator" && (() => {
        ...
        const totalExpenses = (calcExpenseSeeds || 0) + 
        ...
        const chartData = [
          ...
          { name: "Net Profit", Amount: netProfit, fill: netProfit >= 0 ? "#10b981" : "#f43f5e" }
        ];
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
                  Estimate total yield, production costs, and net potential profit. Fill in manual parameters or click a preset crop profile below.
                  <br />
                  <span className="text-emerald-600 dark:text-emerald-400 font-semibold">फसल उत्पादन र आम्दानी क्यालकुलेटर: आफ्नो जग्गा, अपेक्षित उत्पादन र बजार भाउ अनुसार सम्भावित नाफा गणना गर्नुहोस्।</span>
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-2.5">
                {Object.keys(calcCropPresets).map((cropKey) => {
                  ...
                  return (
                    ...
                    >
                      <span className="truncate">{calcCropPresets[cropKey].name}</span>
                      <span className={`text-[10px] font-normal truncate ${isActive ? "text-emerald-100" : "text-slate-400 dark:text-slate-500"}`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-5 space-y-6">
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-xs space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        step="0.1"
                        value={calcLandSize}
                        onChange={(e) => setCalcLandSize(Math.max(0.1, Number(e.target.value) || 0))}
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="custom">Other / Custom Crop (अन्य बाली)</option>
                    </select>
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-xs space-y-4">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-[10px] text-slate-400 dark:text-slate-500 italic">
                        Typical yield: ~{calcCropPresets[calcCrop]?.yieldPerRopani ? Math.round(calcCropPresets[calcCrop].yieldPerRopani * landUnitToRopaniMultiplier[calcLandUnit]) : 500} Kg / {calcLandUnit}
                      </span>
                    </div>
                    <div className="relative">
                      <input
                        ...
                        min="1"
                        value={calcYieldRate}
                        onChange={(e) => setCalcYieldRate(Math.max(1, Number(e.target.value) || 0))}
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-[10px] text-slate-400 dark:text-slate-500 italic">
                        Standard rate: ~{calcCropPresets[calcCrop]?.pricePerKg || 40} NRs / Kg
                      </span>
                    </div>
                    <div className="relative">
                      <input
                        ...
                        onChange={(e) => setCalcMarketRate(Math.max(1, Number(e.target.value) || 0))}
                        className="w-full bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 focus:border-emerald-500 focus:outline-none rounded-xl py-2 px-3 text-sm text-slate-700 dark:text-slate-200 font-mono pr-20"
                      />
                      <span className="absolute right-3 top-2.5 text-xs text-slate-400 font-bold uppercase">
                        NRs. / Kg
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-xs space-y-4">
                  ...
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 leading-normal">
                    Estimated costs scaled proportionally to {calcLandSize} {calcLandUnit}. You can edit these fields manually to reflect your actual seasonal expenditures.
                  </p>
                  ...
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3.5">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setCalcExpenseSeeds(Math.max(0, Number(e.target.value) || 0))}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setCalcExpenseFertilizer(Math.max(0, Number(e.target.value) || 0))}
                          className="w-full bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 focus:border-emerald-500 focus:outline-none rounded-xl py-1.5 px-3 text-xs text-slate-700 dark:text-slate-200 font-mono"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3.5">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setCalcExpenseLabor(Math.max(0, Number(e.target.value) || 0))}
                          className="w-full bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 focus:border-emerald-500 focus:outline-none rounded-xl py-1.5 px-3 text-xs text-slate-700 dark:text-slate-200 font-mono"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setCalcExpenseTractor(Math.max(0, Number(e.target.value) || 0))}
                          className="w-full bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 focus:border-emerald-500 focus:outline-none rounded-xl py-1.5 px-3 text-xs text-slate-700 dark:text-slate-200 font-mono"
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setCalcExpenseTransport(Math.max(0, Number(e.target.value) || 0))}
                        className="w-full bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 focus:border-emerald-500 focus:outline-none rounded-xl py-1.5 px-3 text-xs text-slate-700 dark:text-slate-200 font-mono"
                      />
                    </div>
                  </div>
              ...
              <div className="lg:col-span-7 space-y-6">
                  ...
                  <div className="bg-rose-50/40 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900 rounded-2xl p-5 shadow-xs flex flex-col justify-between">
                    ...
                    <div className="mt-4">
                      ...
                      <span className="text-[10px] text-rose-600/80 dark:text-rose-400/80 font-medium block mt-1">
                        Avg cost: Rs. {Math.round(totalExpenses / calcLandSize).toLocaleString()} / {calcLandUnit}
                      </span>
                    </div>
                  </div>

                  {/* Net Profit */}
                  <div className={`border rounded-2xl p-5 shadow-xs flex flex-col justify-between transition-colors duration-200 ${
                    netProfit >= 0 
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-xs space-y-4">
                  ...
                  {totalExpenses === 0 ? (
                    <p className="text-center py-4 text-xs text-slate-400">No expenses recorded yet.</p>
                  ) : (
                    <div className="space-y-3">
                      ...
                      <div className="h-4 w-full rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden flex">
                        <div 
                          ...
                          style={{ width: `${Math.round((calcExpenseSeeds / totalExpenses) * 100)}%` }} 
                          title={`Seeds: ${Math.round((calcExpenseSeeds / totalExpenses) * 100)}%`}
                        />
                        <div 
                          className="h-full bg-amber-500" 
                          style={{ width: `${Math.round((calcExpenseFertilizer / totalExpenses) * 100)}%` }} 
                          title={`Fertilizer: ${Math.round((calcExpenseFertilizer / totalExpenses) * 100)}%`}
                        />
                        <div 
                          className="h-full bg-blue-500" 
                          style={{ width: `${Math.round((calcExpenseLabor / totalExpenses) * 100)}%` }} 
                          title={`Labor: ${Math.round((calcExpenseLabor / totalExpenses) * 100)}%`}
                        />
                        <div 
                          className="h-full bg-indigo-500" 
                          style={{ width: `${Math.round((calcExpenseTractor / totalExpenses) * 100)}%` }} 
                          title={`Tractor: ${Math.round((calcExpenseTractor / totalExpenses) * 100)}%`}
                        />
                        <div 
                          className="h-full bg-rose-500" 
                          style={{ width: `${Math.round((calcExpenseTransport / totalExpenses) * 100)}%` }} 
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 pt-1">
                        <div className="space-y-0.5">
                          ...
                          <span className="text-xs font-bold text-slate-800 dark:text-slate-200 block font-mono pl-3.5">
                            Rs.{calcExpenseSeeds.toLocaleString()} ({Math.round((calcExpenseSeeds / totalExpenses) * 100)}%)
                          </span>
                        </div>
                        ...
                        <div className="space-y-0.5">
                          ...
                          <span className="text-xs font-bold text-slate-800 dark:text-slate-200 block font-mono pl-3.5">
                            Rs.{calcExpenseFertilizer.toLocaleString()} ({Math.round((calcExpenseFertilizer / totalExpenses) * 100)}%)
                          </span>
                        </div>
                        ...
                        <div className="space-y-0.5">
                          ...
                          <span className="text-xs font-bold text-slate-800 dark:text-slate-200 block font-mono pl-3.5">
                            Rs.{calcExpenseLabor.toLocaleString()} ({Math.round((calcExpenseLabor / totalExpenses) * 100)}%)
                          </span>
                        </div>
                        ...
                        <div className="space-y-0.5">
                          ...
                          <span className="text-xs font-bold text-slate-800 dark:text-slate-200 block font-mono pl-3.5">
                            Rs.{calcExpenseTractor.toLocaleString()} ({Math.round((calcExpenseTractor / totalExpenses) * 100)}%)
                          </span>
                        </div>
                        ...
                        <div className="space-y-0.5">
                          ...
                          <span className="text-xs font-bold text-slate-800 dark:text-slate-200 block font-mono pl-3.5">
                            Rs.{calcExpenseTransport.toLocaleString()} ({Math.round((calcExpenseTransport / totalExpenses) * 100)}%)
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <div className="bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-3.5">
                  <div className="flex items-center space-x-2 text-slate-800 dark:text-slate-200">
                    ...
                    <span className="font-extrabold text-xs uppercase tracking-wider">Agri-Advisory &amp; Profit Recommendations</span>
                  </div>
                    ...
                    {profitMargin >= 45 ? (
                      <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900 rounded-xl p-4 space-y-2">
                        <p className="font-bold text-emerald-900 dark:text-emerald-400 flex items-center space-x-1.5">
                          <span>🌟 Excellent Potential Margin! (उत्कृष्ट खुद नाफा दर)</span>
                        </p>
                        <p className="text-emerald-800 dark:text-emerald-300 font-normal">
                          Your calculated margin of <strong>{profitMargin}%</strong> is extremely viable. This suggests your cultivation density and expected market price are highly optimized.
                        </p>
                        <div className="text-[11px] text-emerald-700 dark:text-emerald-400 space-y-1 pl-1">
                          <p>&bull; <strong>Marketing Strategy:</strong> Direct list this crop on our B2B portal to negotiate long-term supply contracts with bulk hospitality buyers in Kathmandu for stable pricing.</p>
                          <p>&bull; <strong>Reinvestment:</strong> Consider allocating 10% of profit to install drip irrigation or solar poly-tunnel covers to secure off-season premium rates.</p>
                        </div>
                      </div>
                    ) : profitMargin >= 15 ? (
                      <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900 rounded-xl p-4 space-y-2">
                        <p className="font-bold text-amber-900 dark:text-amber-400">
                          ⚖️ Healthy Seasonal Balance (सामान्य नाफाको सम्भावना)
                        </p>
                        <p className="text-amber-800 dark:text-amber-300 font-normal">
                          A <strong>{profitMargin}%</strong> margin matches average regional standards in {user.district} district. It provides steady sustainability but has room for further cost controls.
                        </p>
                        <div className="text-[11px] text-amber-700 dark:text-amber-400 space-y-1 pl-1">
                          <p>&bull; <strong>Labor Optimization:</strong> Labor wages represent a noticeable input block. Explore community mechanization pools or coordinate shared harvest schedules with neighbor cooperatives.</p>
                          <p>&bull; <strong>Logistics:</strong> Coordinate transport with local cooperatives to share flatbed trucks to Kathmandu, slicing travel costs by up to 25%.</p>
                        </div>
                      </div>
                    ) : (
                      <div className="bg-rose-50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900 rounded-xl p-4 space-y-2">
                        <p className="font-bold text-rose-900 dark:text-rose-400 flex items-center space-x-1.5">
                          <span>🚨 Critical Cost Threshold Warning (उच्च जोखिम चेतावनी)</span>
                        </p>
                        <p className="text-rose-800 dark:text-rose-300 font-normal">
                          Calculated profit margin is low (<strong>{profitMargin}%</strong>). At current rates, high input expenditures (seeds, fertilizer, transport) leave you highly sensitive to weather or price fluctuations.
                        </p>
                        <div className="text-[11px] text-rose-700 dark:text-rose-400 space-y-1 pl-1">
                          <p>&bull; <strong>Cost Mitigation:</strong> Seek subsidy programs for seeds and compost fertilizer from Trishuli Valley Farmers Union or Dhading Cooperative network.</p>
                          <p>&bull; <strong>Premium Pricing:</strong> Consider certifying your farm as "Dhading Organic" to list crops with a 15-20% target premium above standard wholesale rates.</p>
                        </div>
                      </div>
                    )}

                    <div className="pt-2 text-[10px] text-slate-400 dark:text-slate-500 italic border-t border-slate-200/50">
                      Note: Calculations are based on regional pilot district variables. Actual yields can fluctuate based on localized micro-climate indices, soil nutrient scores, and pest mitigation. Use as guidance only.
                    </div>
                  </div>
                </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-slate-900 via-emerald-950 to-slate-900 text-white rounded-2xl p-6 shadow-md border border-slate-800">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-slate-300 max-w-2xl leading-relaxed">
                  {t("Log of previous QR codes scanned, including cooperative member digital passes, harvest batch crate tags, and raw codes.")}
                </p>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-5 mt-5 border-t border-white/10">
              <div className="bg-white/5 border border-white/10 rounded-xl p-3">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t("Total Scans")}</p>
                <p className="text-xl font-black font-mono text-white mt-0.5">{scanLogs.length}</p>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-xl p-3">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t("Member Passes")}</p>
                <p className="text-xl font-black font-mono text-emerald-400 mt-0.5">
                  {scanLogs.filter(s => s.scanType === "member").length}
                </p>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-xl p-3">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t("Harvest Batches")}</p>
                <p className="text-xl font-black font-mono text-indigo-400 mt-0.5">
                  {scanLogs.filter(s => s.scanType === "batch").length}
                </p>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-xl p-3">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t("Latest Activity")}</p>
                <p className="text-xs font-bold font-mono text-amber-300 mt-1 truncate">
                  {scanLogs.length > 0 ? new Date(scanLogs[0].scannedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : t("No activity")}
                </p>
              </div>
            </div>
          </div>
          ...
          {scanActionSuccess && (
            <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl p-3.5 text-xs font-bold flex items-center justify-between animate-in fade-in">
              ...
              <button onClick={() => setScanActionSuccess("")} className="text-emerald-700 hover:text-emerald-900 cursor-pointer">
                <X className="w-3.5 h-3.5" />
              </button>
            </div>
          )}
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center space-x-1.5 overflow-x-auto pb-1 sm:pb-0">
              ...
              >
                {t("Member Passes")} ({scanLogs.filter(s => s.scanType === "member").length})
              </button>
              <button
              ...
              >
                {t("Harvest Batches")} ({scanLogs.filter(s => s.scanType === "batch").length})
              </button>
              <button
              ...
              >
                {t("Text / Code")} ({scanLogs.filter(s => s.scanType === "text").length})
              </button>
            </div>
          ...
          {isFetchingScanLogs ? (
            <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center space-y-3">
              ...
              <p className="text-xs text-slate-500 font-semibold">{t("Loading QR scan history logs...")}</p>
            </div>
          ) : (() => {
            const filteredScans = scanLogs.filter(item => {
              if (scanFilter !== "all" && item.scanType !== scanFilter) return false;
              if (scanSearch.trim()) {
                const q = scanSearch.toLowerCase();
                const titleMatch = item.title.toLowerCase().includes(q);
                const detailsMatch = item.details.toLowerCase().includes(q);
                const metaMatch = JSON.stringify(item.metadata || {}).toLowerCase().includes(q);
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center space-y-4 shadow-sm">
                  ...
                  <div className="max-w-md mx-auto space-y-1">
                    <h3 className="font-bold text-slate-800 text-sm">
                      {scanSearch || scanFilter !== "all" ? t("No matching scan records found") : t("No QR scans recorded yet")}
                    </h3>
                    <p className="text-xs text-slate-500 leading-relaxed">
                      {scanSearch || scanFilter !== "all"
                        ? t("Try adjusting your search terms or filter selection.")
                        : t("Use the Mobile Camera QR Scanner to scan produce batch tags or cooperative member passes. All scan activity will be logged here.")}
                    </p>
                  </div>
                  <button
                    onClick={() => setIsQrModalOpen(true)}
                    ...
                    >
                        ...
                        <div className="space-y-1.5">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                              ...
                              {log.scanType === "member" ? t("Member Pass") : log.scanType === "batch" ? t("Harvest Batch") : t("Code / Text")}
                            </span>
                          </div>
                      ...
                      <div className="flex items-center space-x-2 shrink-0 self-end sm:self-center">
                        {log.scanType === "batch" && log.metadata?.crop && (
                          <button
                            onClick={() => handleQrPreFillForm(log.metadata?.crop || "", log.metadata?.quantity || "", log.metadata?.price || "")}
                        ...
                        {log.scanType === "member" && (
                          <button
                            onClick={() => handleOpenContactModal(log.metadata?.crop || "General Coordinate")}
              ...
              <div>
                ...
                {coops.length === 0 ? (
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-amber-800 text-xs flex items-start space-x-2">
                    ...
                    <div>
                      <p className="font-bold">No registered cooperatives found in {user.district}.</p>
                      <p className="mt-0.5 text-amber-700 leading-normal">
                        Please register with a local office first, or contact our administrative hub for help.
                      </p>
                    </div>
                  </div>
                ) : (
              ...
              <div className="flex items-center space-x-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-rose-800 mt-0.5">
                    {t("Critical precautions for")} {t(weatherAlertDistrict)} {t("District")}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-5 overflow-y-auto max-h-[70vh]">
              ...
              <div className="bg-rose-50/50 border border-rose-100 rounded-xl p-4 space-y-2">
                <p className="text-xs font-bold text-rose-900">
                  {t("⚠️ Primary Hazards: Mudslides & Landslides (पहिरोको जोखिम)")}
                </p>
                <p className="text-xs text-rose-800 leading-relaxed font-normal">
                  {t("Both Dhading and Makwanpur feature steep hilly topography. Hilly slopes and terraced farmlands can instantly liquefy under prolonged extreme rainfall, washing away topsoil, crop beds, or farm infrastructure.")}
                </p>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="border border-slate-150 rounded-xl p-4 bg-slate-50 space-y-2">
                    ...
                    <h5 className="text-xs font-bold text-slate-800">{t("1. Clear Run-off Channels")}</h5>
                    <p className="text-[11px] text-slate-600 leading-relaxed">
                      {t("Walk the perimeter of tomato trellis plots and potato ridges. Manually shovel out soil blockages in the arterial drain channels to divert heavy volumes of rainwater away from crop roots.")}
                    </p>
                  </div>

                  <div className="border border-slate-150 rounded-xl p-4 bg-slate-50 space-y-2">
                    <span className="text-lg">🍅</span>
                    <h5 className="text-xs font-bold text-slate-800">{t("2. Safeguard Harvested Stocks")}</h5>
                    <p className="text-[11px] text-slate-600 leading-relaxed">
                      {t("Move all wooden or plastic vegetable crates off bare earth fields. Store harvested crops inside secure, elevated brick or concrete storage warehouses immediately.")}
                    </p>
                  </div>

                  <div className="border border-slate-150 rounded-xl p-4 bg-slate-50 space-y-2">
                    <span className="text-lg">🎒</span>
                    <h5 className="text-xs font-bold text-slate-800">{t("3. Human Life Safety First")}</h5>
                    <p className="text-[11px] text-slate-600 leading-relaxed">
                      {t("If your fields are located near high landslide hazard zones (such as steep valley banks in Malekhu, Dhading or Sisneri, Makwanpur), prioritize evacuating farm labor to safe community shelters.")}
                    </p>
                  </div>

                  <div className="border border-slate-150 rounded-xl p-4 bg-slate-50 space-y-2">
                    <span className="text-lg">🛣️</span>
                    <h5 className="text-xs font-bold text-slate-800">{t("4. Suspend Highway Transport")}</h5>
                    <p className="text-[11px] text-slate-600 leading-relaxed">
                      {t("Highway landslide blockages on the Prithvi Highway or Kanti Lokpath can leave perishable vegetable trucks stranded for days. Check current police road closure reports before loading cargo.")}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-slate-100 pt-4 space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div className="bg-slate-50 p-3 rounded-lg border border-slate-150">
                    <p className="font-bold text-slate-700">{t("Dhading District Emergency Hub")}</p>
                    <p className="text-[11px] text-slate-500 mt-0.5">{t("📞 Landslide & Flood Response: +977-10-520133")}</p>
                  </div>
                  <div className="bg-slate-50 p-3 rounded-lg border border-slate-150">
                    <p className="font-bold text-slate-700">{t("Makwanpur District Emergency Hub")}</p>
                    <p className="text-[11px] text-slate-500 mt-0.5">{t("📞 Landslide & Flood Response: +977-57-520112")}</p>
                  </div>
                </div>
              </div>
