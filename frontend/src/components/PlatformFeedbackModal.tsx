...
import { User, FeedbackType, FeedbackPriority, PlatformFeedback } from "../types";
import { 
  X, Send, Sparkles, AlertTriangle, Lightbulb, MessageSquare, 
  CheckCircle2, Clock, ShieldCheck, Tag, ThumbsUp, HelpCircle, 
  UserCheck, MapPin, Phone, Layers, ChevronRight, Eye, RefreshCw
} from "lucide-react";
...
export default function PlatformFeedbackModal({ isOpen, onClose, user, token }: FeedbackModalProps) {
  const { t } = useLanguage();
  ...
  const [userDistrict, setUserDistrict] = useState(user?.district || "Kathmandu");
  const [userPhone, setUserPhone] = useState(user?.phone || "");

  // Submission & Data State
  const [submitting, setSubmitting] = useState(false);
  const [submitError, setSubmitError] = useState("");
  const [submittedFeedback, setSubmittedFeedback] = useState<PlatformFeedback | null>(null);
  ...
  // Sync user details if user prop updates
  useEffect(() => {
    if (user) {
  ...
  // Fetch community feedback list when explore tab is opened
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title.trim()) {
      setSubmitError(t("Please provide a title for your feedback or feature request."));
      return;
    }
    if (!description.trim() || description.trim().length < 10) {
      setSubmitError(t("Please enter a detailed description (at least 10 characters)."));
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSubmitError(err.error || t("Failed to submit platform feedback. Please try again."));
      }
    } catch (err) {
      setSubmitError(t("Network communication error. Please check connection."));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    switch (fbType) {
      ...
      case "usability_issue":
        return {
          ...
          icon: MessageSquare,
          style: "bg-purple-50 dark:bg-purple-950/40 text-purple-700 dark:text-purple-300 border-purple-200 dark:border-purple-800"
        };
      default:
        return {
  ...
  const getStatusBadge = (st: string) => {
    switch (st) {
      case "resolved":
        return { text: t("Resolved / Deployed (सम्पन्न)"), style: "bg-emerald-100 dark:bg-emerald-900/50 text-emerald-800 dark:text-emerald-300 border-emerald-200" };
      case "in_progress":
        return { text: t("In Progress (काम भइरहेको)"), style: "bg-indigo-100 dark:bg-indigo-900/50 text-indigo-800 dark:text-indigo-300 border-indigo-200" };
      case "under_review":
        return { text: t("Under Review (समीक्षाधीन)"), style: "bg-amber-100 dark:bg-amber-900/50 text-amber-800 dark:text-amber-300 border-amber-200" };
      case "dismissed":
        return { text: t("Closed"), style: "bg-slate-100 dark:bg-slate-800 text-slate-500 border-slate-200" };
      default:
        return { text: t("Pending Admin Review (प्रतीक्षारत)"), style: "bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200" };
    }
  };
      ...
      >
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {t("Submit requests or report issues directly to the AgriTech administrative engineering team")}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-6 pt-3 pb-0 bg-white dark:bg-slate-900 border-b border-slate-150 dark:border-slate-800 flex space-x-4">
          ...
          >
            ...
            <span>{t("Community Roadmap & Requests")} ({feedbackList.length})</span>
          </button>
        </div>
                ...
                <div className="bg-emerald-50/80 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/80 rounded-xl p-6 text-center space-y-4 animate-in zoom-in-95 duration-200">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-emerald-700 dark:text-emerald-300 max-w-md mx-auto mt-1 leading-relaxed">
                      {t("Thank you for contributing to the long-term enhancement of AgriTech Nepal. Our administrative engineering team evaluates submissions regularly.")}
                    </p>
                  </div>
                  ...
                  <div className="bg-white dark:bg-slate-900 border border-emerald-200 dark:border-emerald-800 rounded-lg p-4 text-left space-y-2 max-w-lg mx-auto">
                    <div className="flex items-center justify-between text-xs font-mono text-slate-500">
                      <span>{t("Reference ID")}: <strong className="text-emerald-600 font-bold">{submittedFeedback.id}</strong></span>
                      <span>{new Date(submittedFeedback.created_at).toLocaleDateString()}</span>
                    </div>
                    ...
                    <div className="flex items-center space-x-2 pt-2 border-t border-slate-100 dark:border-slate-800 text-[10px] text-slate-500">
                      <span className="capitalize">{t("Type")}: {submittedFeedback.type.replace('_', ' ')}</span>
                      <span>•</span>
                      <span className="capitalize">{t("Priority")}: {submittedFeedback.priority}</span>
                      <span>•</span>
                      <span className="text-emerald-600 font-semibold">{t("Status")}: {submittedFeedback.status}</span>
                    </div>
                  </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2">
                      1. {t("Select Feedback Category")} <span className="text-rose-500">*</span>
                    </label>
                      ...
                      {[
                        {
                          id: "feature_request",
                          label: t("Feature Request (नयाँ विशेषता)"),
                          desc: t("Suggest new functionality, reports, or automation"),
                          icon: Lightbulb,
                        ...
                        },
                        {
                          ...
                          desc: t("Report glitches, calculation errors, or system issues"),
                          icon: AlertTriangle,
                        ...
                        },
                        {
                          id: "usability_issue",
                          label: t("Usability & Design (डिजाइन अनुभव)"),
                          desc: t("Feedback on layout, mobile view, or ease of use"),
                          icon: MessageSquare,
                        ...
                        },
                        {
                          ...
                          desc: t("General comments or service recommendations"),
                          icon: Sparkles,
                          color: "border-emerald-300 bg-emerald-50/50 dark:bg-emerald-950/20 text-emerald-900 dark:text-emerald-200"
                        }
                      ].map(cat => {
                        const Icon = cat.icon;
                        ...
                        return (
                          ...
                          >
                            <div className={`p-2 rounded-lg ${isSelected ? "bg-white/80 dark:bg-slate-800/80" : "bg-slate-100 dark:bg-slate-800 text-slate-500"}`}>
                              <Icon className="w-4 h-4" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="font-bold text-xs text-slate-900 dark:text-white leading-tight">{cat.label}</div>
                              <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-2">{cat.desc}</div>
                            </div>
                          </div>
                        );
                      })}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div className="sm:col-span-1">
                      ...
                      >
                        ...
                        <option value="medium">{t("Medium (मध्यम Priority)")}</option>
                        <option value="high">{t("High (उच्च Priority)")}</option>
                        <option value="critical">{t("Critical (गम्भीर Priority)")}</option>
                      </select>
                    </div>
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1.5">
                        3. {t("Title / Summary")} <span className="text-rose-500">*</span>
                      </label>
                      <input
                        type="text"
                        value={title}
                        onChange={e => setTitle(e.target.value)}
                        placeholder={t("e.g. Add SMS alerts for buyer counter-offers")}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1.5">
                      4. {t("Detailed Explanation")} <span className="text-rose-500">*</span>
                    </label>
                    <textarea
                      rows={4}
                      value={description}
                      onChange={e => setDescription(e.target.value)}
                      placeholder={t("Describe the feature request or issue in detail. For bug reports, mention steps to reproduce or expected vs actual outcome.")}
                  ...
                  <div className="bg-slate-50 dark:bg-slate-850 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                      <div>
                        <label className="block text-[10px] font-semibold text-slate-500 mb-1">{t("Full Name")}</label>
                        <input
                          type="text"
                          value={userName}
                      ...
                      <div>
                        <label className="block text-[10px] font-semibold text-slate-500 mb-1">{t("District")}</label>
                        <input
                          type="text"
                          value={userDistrict}
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-50 dark:bg-slate-850 p-3 rounded-xl border border-slate-200 dark:border-slate-800 text-xs">
                <div className="flex items-center space-x-2">
                  <span className="font-bold text-slate-600 dark:text-slate-400">{t("Category")}:</span>
                  <select
                    value={filterType}
                  ...
                  >
                    ...
                    <option value="feature_request">{t("Feature Requests")}</option>
                    <option value="bug_report">{t("Bug Reports")}</option>
                    <option value="usability_issue">{t("Usability & Design")}</option>
                    <option value="general_feedback">{t("General Suggestions")}</option>
                  </select>
                </div>

                <div className="flex items-center space-x-2">
                  <span className="font-bold text-slate-600 dark:text-slate-400">{t("Status")}:</span>
                  <select
                    value={filterStatus}
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-center space-x-2">
                            <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold border flex items-center space-x-1 ${badge.style}`}>
                              <Icon className="w-3 h-3" />
                              <span>{badge.label}</span>
                            </span>
                            <span className={`px-2 py-0.5 rounded-full text-[10px] uppercase tracking-wider ${getPriorityBadge(item.priority)}`}>
                              {item.priority}
                            </span>
                          </div>
                          <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${stBadge.style}`}>
                            {stBadge.text}
                          </span>
                        </div>
                        ...
                        {item.adminNotes && (
                          <div className="bg-emerald-50/70 dark:bg-emerald-950/30 border border-emerald-200/80 dark:border-emerald-800/80 rounded-lg p-2.5 text-xs space-y-1">
                            <div className="font-bold text-[10px] uppercase text-emerald-800 dark:text-emerald-300 flex items-center space-x-1">
                              ...
                              <span>{t("AgriTech Administrative Response")}</span>
                            </div>
                        ...
                        <div className="flex items-center justify-between text-[10px] text-slate-400 dark:text-slate-500 pt-2 border-t border-slate-100 dark:border-slate-800">
                          <span className="flex items-center space-x-1">
                            <span>{item.userName || "User"} ({t(item.userRole || "guest")})</span>
                            {item.userDistrict && <span>• {t(item.userDistrict)}</span>}
                          </span>
        ...
        <div className="px-6 py-3.5 border-t border-slate-150 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-850/80 text-[11px] text-slate-500 dark:text-slate-400 flex justify-between items-center">
          <span className="flex items-center space-x-1.5">
            ...
            <span>{t("Direct line to AgriTech Administrative Team • Kathmandu")}</span>
          </span>
          <button
            onClick={onClose}
