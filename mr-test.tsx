...
export default function MarketPricesView({
    ...
    try {
      if (activeToken) {
        ...
        if (res.ok) {
          const data = await res.json();
          setAlertSuccess(
            `Price alert saved! You will receive visual alerts when ${alertCrop} market rate is ${alertCriteria} NRs. ${alertThreshold}/KG.`,
          );
          fetchAlertsAndNotifications();
        } else {
  ...
  const handleDeleteAlert = async (id: string) => {
    try {
      if (activeToken) {
        const res = await fetch(
          `http://127.0.0.1:8000/api/market-price/alerts/${id}`,
          {
            method: "DELETE",
            headers: { Authorization: `Bearer ${activeToken}` },
          },
    ...
    try {
      if (activeToken) {
        ...
        if (res.ok) {
          setPriceAlerts((prev) =>
            prev.map((a) =>
              a.id === id ? { ...a, isActive: !currentIsActive } : a,
            ),
          );
        }
      } else {
  ...
  const handleSimulateSurge = async () => {
    ...
    try {
      if (activeToken) {
        const res = await fetch(
          "http://127.0.0.1:8000/api/market-price/alerts/test-trigger",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (res.ok) {
          const data = await res.json();
          setSurgeSimulationSuccess(
            `Simulated price surge evaluated! ${data.triggeredCount || 0} alert(s) triggered and logged.`,
          );
          fetchAlertsAndNotifications();
        }
      } else {
        setSurgeSimulationSuccess(
          `Simulated rate of NRs. ${simulatingSurgePrice}/KG tested against saved alerts!`,
        );
      }
  ...
  const getTriggeredAlertsForPrice = (p: MarketPrice) => {
    return priceAlerts.filter((a) => {
      ...
      const cropMatch =
        p.crop.toLowerCase().includes(a.crop.toLowerCase()) ||
        a.crop.toLowerCase().includes(p.crop.toLowerCase());
      const regionMatch =
        !a.region ||
        a.region === "all" ||
        p.region.toLowerCase() === a.region.toLowerCase();
  ...
  const allTriggeredAlerts = priceAlerts.filter((alert) => {
    ...
    return prices.some((p) => {
      const cropMatch =
        p.crop.toLowerCase().includes(alert.crop.toLowerCase()) ||
        alert.crop.toLowerCase().includes(p.crop.toLowerCase());
      const regionMatch =
        !alert.region ||
        alert.region === "all" ||
        p.region.toLowerCase() === alert.region.toLowerCase();
      if (!cropMatch || !regionMatch) return false;
  ...
  const getWeeklyTrend = (currentPrice: MarketPrice) => {
    ...
    if (history.length <= 1) {
      return {
        ...
        diff: 0,
        oldPrice: currentPrice.price_per_unit,
      };
    }
    ...
    for (const h of history) {
      ...
      const daysDiff =
        (curDate.getTime() - hDate.getTime()) / (1000 * 3600 * 24);
    ...
    if (!bestMatch) {
      ...
      for (const h of history) {
        ...
        const hDate = new Date(h.date);
        const daysDiff =
          (hDate.getTime() - curDate.getTime()) / (1000 * 3600 * 24);
        if (daysDiff > 0.5) {
  ...
  const DISTRICTS = [
    { id: "all", name: "All Districts", marketName: "National Index" },
    {
      id: "Kathmandu",
      name: "Kathmandu",
      marketName: "Kalimati & Tokha Mandi",
    },
    {
      id: "Dhading",
      name: "Dhading",
      marketName: "Dhading Wholesale & Malekhu",
    },
    {
      id: "Makwanpur",
      name: "Makwanpur",
      marketName: "Hetauda Mandi & Palung",
    },
    {
      id: "Chitwan",
      name: "Chitwan / Terai",
      marketName: "Narayangarh & Terai Mandi",
    },
  ];
  ...
  let filteredPrices = prices.filter((p) => {
    ...
    if (searchQuery.trim()) {
      ...
      const matchDistrict =
        ...
        getPriceDistrict(p).toLowerCase().includes(q) ||
    ...
    if (filterCrop) {
      if (!p.crop.toLowerCase().includes(filterCrop.toLowerCase()))
        return false;
    }
    ...
    if (
      pricingTier === "50-100" &&
      (p.price_per_unit < 50 || p.price_per_unit > 100)
    )
      return false;
  ...
  let directoryPrices = filteredPrices.filter(
    (p) => !latestDateInDb || p.date === latestDateInDb,
  );
  ...
  const minPrice =
    chartData.length > 0 ? Math.min(...chartData.map((item) => item.price)) : 0;
  const maxPrice =
    chartData.length > 0 ? Math.max(...chartData.map((item) => item.price)) : 0;
  ...
  const pCropA = prices.filter(
    (p) =>
      p.crop === compareCropA &&
      (compareDistrict === "all" || getPriceDistrict(p) === compareDistrict),
  );
  const pCropB = prices.filter(
    (p) =>
      p.crop === compareCropB &&
      (compareDistrict === "all" || getPriceDistrict(p) === compareDistrict),
  );
  ...
  const avgCropA =
    pCropA.length > 0
      ? Math.round(
          pCropA.reduce((sum, item) => sum + item.price_per_unit, 0) /
            pCropA.length,
        )
      : 0;
  const avgCropB =
    pCropB.length > 0
      ? Math.round(
          pCropB.reduce((sum, item) => sum + item.price_per_unit, 0) /
            pCropB.length,
        )
      : 0;
  const priceDiff = avgCropB - avgCropA;
  const priceDiffPct =
    avgCropA > 0 ? Math.round(((avgCropB - avgCropA) / avgCropA) * 100) : 0;
  ...
  const sideBySideTableRows = comparisonDistrictsList.map((dist) => {
    const pricesAInDist = prices.filter(
      (p) => p.crop === compareCropA && getPriceDistrict(p) === dist.id,
    );
    const pricesBInDist = prices.filter(
      (p) => p.crop === compareCropB && getPriceDistrict(p) === dist.id,
    );

    const rateA =
      pricesAInDist.length > 0
        ? Math.round(
            pricesAInDist.reduce((s, x) => s + x.price_per_unit, 0) /
              pricesAInDist.length,
          )
        : 0;
    const rateB =
      pricesBInDist.length > 0
        ? Math.round(
            pricesBInDist.reduce((s, x) => s + x.price_per_unit, 0) /
              pricesBInDist.length,
          )
        : 0;
  ...
  const allComparisonDates = Array.from(
    new Set(prices.map((p) => p.date as string)),
  ).sort();
  const dualCropChartData = allComparisonDates
    .map((dateStr: string) => {
      const listA = prices.filter(
        (p) =>
          p.crop === compareCropA &&
          p.date === dateStr &&
          (compareDistrict === "all" ||
            getPriceDistrict(p) === compareDistrict),
      );
      const listB = prices.filter(
        (p) =>
          p.crop === compareCropB &&
          p.date === dateStr &&
          (compareDistrict === "all" ||
      ...
      const priceAOnDate =
        listA.length > 0
          ? Math.round(
              listA.reduce((s, x) => s + x.price_per_unit, 0) / listA.length,
            )
          : null;
      const priceBOnDate =
        listB.length > 0
          ? Math.round(
              listB.reduce((s, x) => s + x.price_per_unit, 0) / listB.length,
            )
          : null;
    ...
    .filter(
      (item) => item[compareCropA] !== null || item[compareCropB] !== null,
    );
              ...
              <div>
                <div className="flex items-center space-x-2">
                  ...
                  <span className="px-2 py-0.5 bg-amber-500 text-slate-950 font-bold text-[10px] rounded-full uppercase tracking-wider shadow-2xs">
                    {allTriggeredAlerts.length} Crop Alert
                    {allTriggeredAlerts.length > 1 ? "s" : ""}
                  </span>
                </div>
                <p className="text-xs text-slate-700 mt-1 leading-relaxed">
                  ...
                  has reached or exceeded your target threshold of NRs.{" "}
                  {allTriggeredAlerts[0]?.priceThreshold}/KG in current market
                  records!
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 pb-3 mb-4 space-y-2 sm:space-y-0">
              ...
              <div className="flex items-center space-x-2 text-xs">
                ...
                >
                  ...
                  <option value="Hill">
                    Hill Districts (Dhading/Makwanpur)
                  </option>
            ...
            <div className="h-56 mt-2">
              {chartData.length > 0 ? (
                ...
                <div className="h-full flex items-center justify-center text-slate-400 text-xs">
                  No historical data found for {selectedChartCrop} in{" "}
                  {selectedChartRegion}.
                </div>
              )}
            </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="under-50">
                      Budget (Under NRs. 50 / KG)
                    </option>
                    <option value="50-100">
                      Mid-Range (NRs. 50 - 100 / KG)
                    </option>
                    <option value="over-100">
                      Premium (Over NRs. 100 / KG)
                    </option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="low-to-high">
                      Wholesale Rate: Low to High
                    </option>
                    <option value="high-to-low">
                      Wholesale Rate: High to Low
                    </option>
                  </select>
                </div>
              </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Quickly find specific crops, agricultural cooperatives, wholesale
              mandis, or category types by name or location.
            </p>
          </div>
          <div className="flex items-center space-x-2">
          ...
          {justRefreshed && (
            ...
            >
              <div className="flex items-center space-x-2">
                ...
                <span>
                  Live market rates refreshed successfully!{" "}
                  {lastRefreshedTime && `(Updated at ${lastRefreshedTime})`}
                </span>
              </div>
        ...
        <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Live rates collected daily from wholesale markets
            </p>
          </div>
          <button
            onClick={fetchPrices}
            ...
            <tbody className="divide-y divide-slate-100 text-slate-600">
              {directoryPrices.length === 0 ? (
                ...
                directoryPrices.map((p, idx) => {
                  ...
                  const savedAlert = priceAlerts.find(
                    (a) =>
                      a.crop.toLowerCase().includes(p.crop.toLowerCase()) ||
                      p.crop.toLowerCase().includes(a.crop.toLowerCase()),
                  );
                        ...
                        <div className="flex flex-col">
                          ...
                          {isTriggered && (
                            ...
                            >
                              ...
                              <span>
                                Threshold Exceeded (Target ≤ NRs.{" "}
                                {triggeredAlerts[0].priceThreshold})
                              </span>
                            </motion.span>
                          )}
                        </div>
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <div className="flex items-center justify-end space-x-1.5">
                          <button
                            ...
                            onClick={() => {
                              addToCart({
                                ...
                                farmerId:
                                  "farmer_" + (getPriceDistrict(p) || "Nepal"),
                                farmerName:
                                  (getPriceDistrict(p) || "Nepal") +
                                  " Smallholder Farm",
                          ...
                          >
                            ...
                            <span>
                              {savedAlert
                                ? `Alert (${savedAlert.priceThreshold})`
                                : "Set Alert"}
                            </span>
                          </button>
                        </div>
                ...
                <div className="flex items-center space-x-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400">
                      Set threshold triggers &amp; receive visual notifications
                      on rate surges
                    </p>
                  </div>
                </div>
              ...
              <div className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <select
                          value={alertCriteria}
                          onChange={(e) =>
                            setAlertCriteria(e.target.value as any)
                          }
                          className="w-full bg-white border border-slate-200 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-xl py-2 px-3 text-xs text-slate-800 font-semibold"
                        >
                          <option value="above">
                            Price Exceeds Target (≥ Threshold)
                          </option>
                          <option value="below">
                ...
                <div className="space-y-3">
                  ...
                  {priceAlerts.length === 0 ? (
                    <div className="text-center py-6 text-slate-400 text-xs italic bg-slate-50 rounded-xl border border-slate-100">
                      No active price alerts saved yet. Use the form above or
                      click "Set Alert" on any crop listing in the directory
                      table.
                    </div>
                  ) : (
                    <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                      {priceAlerts.map((alert) => {
                        ...
                        const currentMarketItem = prices.find((p) =>
                          p.crop
                            .toLowerCase()
                            .includes(alert.crop.toLowerCase()),
                        );
                        const currentPrice = currentMarketItem
                          ? currentMarketItem.price_per_unit
                          : null;
                          ...
                          >
                            <div className="space-y-1">
                              <div className="flex items-center space-x-2">
                                ...
                                <span
                                  className={`px-2 py-0.25 text-[10px] font-bold rounded-full ${
                                    isTriggered
                ...
                <div className="bg-slate-900 text-white rounded-xl p-4 space-y-3">
                  ...
                  <p className="text-xs text-slate-300 leading-relaxed">
                    Test your price alert configurations by simulating a market
                    rate surge in wholesale markets.
                  </p>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-2">
                    <div className="relative flex-1 w-full">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setSimulatingSurgePrice(e.target.value)
                        }
                    ...
                    >
                      <RefreshCw
                        className={`w-3.5 h-3.5 ${isSimulatingSurge ? "animate-spin" : ""}`}
                      />
                      <span>
                        {isSimulatingSurge
                          ? "Testing..."
